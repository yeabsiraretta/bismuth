...
function summarizeBucket(
  ...
  const lines = limitLines(
    filtered.map((entry, index) => {
      ...
      const text =
        entry.content_summary || entry.snippet || entry.section_heading || entry.source_path;
...
function conflictWarnings(entries: SearchResult[]): string[] {
  ...
  for (const entry of entries) {
    const text =
      `${entry.section_heading ?? ''} ${entry.content_summary ?? ''} ${entry.snippet ?? ''} ${entry.tags ?? ''}`.toLowerCase();
    if (
...
export async function generateMemorySynthesis(
  ...
  const items = results.map((entry, index) =>
    entryToSynthesisItem(entry, `${categoryPrefix(fileCategory(entry))}${index + 1}`)
  );
  const sections: string[] = [];
  sections.push(
    buildCurrentScope(
      featureName,
      path.relative(projectRoot, featureRoot),
      featureSpec,
      featureMemory
    )
  );
  ...
  sections.push(
    summarizeBucket('Active Architecture Constraints', results, 'architecture', 5, 'A')
  );
...
export async function writeMemorySynthesis(
  ...
  const synthesis = await generateMemorySynthesis(
    db,
    projectRoot,
    featurePath,
    config,
