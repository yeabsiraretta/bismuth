...
export function deleteSourceEntries(db: MemoryDatabase, sourcePath: string): void {
  const ids = db
    .prepare(`SELECT id FROM memory_entries WHERE source_path = ?`)
...
export function loadAllEntries(db: MemoryDatabase): MemoryEntryRecord[] {
  return db
    .prepare(`SELECT * FROM memory_entries ORDER BY source_path, line_start ASC, line_end ASC`)
    .all() as MemoryEntryRecord[];
}

export function loadEntriesBySourcePath(
  db: MemoryDatabase,
  sourcePath: string
): MemoryEntryRecord[] {
  return db
    .prepare(
      `SELECT * FROM memory_entries WHERE source_path = ? ORDER BY line_start ASC, line_end ASC`
    )
    .all(sourcePath) as MemoryEntryRecord[];
}
...
export function loadDistinctSourcePaths(db: MemoryDatabase): string[] {
  const rows = db.prepare(`SELECT DISTINCT source_path FROM memory_entries`).all() as Array<{
    source_path: string;
  }>;
