...
async function ensureIndexedMemory(
  projectRoot: string,
  db: any,
...
async function runSynthesize(
  ...
  try {
    ...
    console.log(
      `Wrote ${path.relative(projectRoot, synthesis.outputPath)} (${synthesis.words} words, ${synthesis.sourceItems.length} source items)`
    );
  } finally {
    closeDatabase(db);
    freeTokenizer();
...
async function runRebuildMemory(projectRoot: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Rebuilt cache from ${result.indexedFiles} files (${result.indexedEntries} entries)`
      );
    } finally {
...
async function runTokenReport(projectRoot: string, featurePath: string): Promise<void> {
  ...
  try {
    ...
    const featureSpec = (await pathExists(featureSpecPath))
      ? await readTextFile(featureSpecPath)
      : '';
    const featureMemory = (await pathExists(featureMemoryPath))
      ? await readTextFile(featureMemoryPath)
      : '';
    ...
    const baselineTokens = baselineSources.reduce(
      (sum, file) => sum + estimateTokens(file.content),
      0
    );

    const selectedResults = searchMemoryEntries(
      db,
      [featureSpec, featureMemory, path.basename(featureRoot)].join('\n'),
      config
    );
    const selectedTexts = selectedResults.map((entry) =>
      [entry.source_path, entry.section_heading, entry.content_summary, entry.snippet, entry.tags]
        .filter(Boolean)
        .join('\n')
    );
    ...
    const selectedTokens =
      selectedTexts.reduce((sum, text) => sum + estimateTokens(text), 0) + indexTokens;
    ...
    printKeyValue(
      'Optimized (index + selected entries):',
      `${selectedTokens.toLocaleString()} estimated tokens`
    );
    ...
    console.log(
      chalk.gray('This is an estimated token count, not guaranteed provider billing usage.')
    );
...
async function runRegisterMemory(
  ...
  if (options.content) {
    ...
    console.log(
      `  Writing entry content to ${options.file} via Node.js [${mode}] (LLM file-edit bypassed).`
    );
  }
  const { config, db } = createContext(projectRoot);
  try {
...
export async function runCli(argv = process.argv): Promise<void> {
  ...
  program
    ...
    .action(() => {
      console.log(
        'flush-memory is planned but not yet implemented. Use rebuild-memory to clear and reindex.'
      );
    });
  ...
  program
    ...
    .action(() => {
      console.log(
        'doctor is planned but not yet implemented. Verify: Node.js >= 18, npm available, better-sqlite3 installed.'
      );
    });
