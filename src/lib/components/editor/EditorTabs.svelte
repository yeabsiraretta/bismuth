...
{#if $editorTabs.length > 0}
  <div
    class="editor-tabs"
    class:vertical={$tabOrientation === 'vertical'}
    class:horizontal={$tabOrientation === 'horizontal'}
    role="tablist"
    ...
    {#each [...groupedTabs.entries()] as [groupId, tabs] (groupId ?? '__ungrouped')}
      ...
      {#if !group?.collapsed}
        {#each tabs as tab (tab.id)}
          <div
            class="tab-item"
            class:active={tab.id === $activeEditorTabId}
...
{#if ctxTab}
  <div class="ctx-menu" style="left:{ctxX}px;top:{ctxY}px" role="menu">
    ...
    <button
      ...
      role="menuitem"
      on:click={() => {
        closeTabsToLeft(ctxTab.id);
        closeCtx();
      }}>Close to Left</button
    >
    ...
    {#each $tabGroups as g (g.id)}
      {#if g.id !== ctxTab?.groupId}
        <button class="ctx-item" role="menuitem" on:click={() => handleMoveToGroup(g.id)}
          >Move to {g.name}</button
        >
      {/if}
    {/each}
    {#if ctxTab.groupId}
...
<style>
  ...
  .group-header {
    ...
    background: none;
    border: none;
    border-left: 3px solid var(--grp-color, var(--text-muted));
    cursor: pointer;
  ...
  .vertical .tab-item.active {
    border-left-color: var(--tab-active-indicator);
  ...
  .ctx-menu {
    ...
    box-shadow: var(--shadow-l, 0 8px 24px rgba(0, 0, 0, 0.15));
  }
  .ctx-item {
    display: block;
    width: 100%;
