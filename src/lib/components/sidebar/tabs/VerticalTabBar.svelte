<script lang="ts">
  ...
  function selectTab(tabId: string) {
    const next = dispatchTabSelect(tabId, activeTab, {
      onCommandsClick,
      onSettingsClick,
      onAboutClick,
      onDailyNoteClick,
  ...
  function handleDragStart(e: DragEvent, tabId: string, source: 'upper' | 'lower') {
    ...
    if (result) {
      draggedTabId = result.draggedTabId;
      dragSource = result.dragSource;
    }
  }
  ...
  function handleDrop(e: DragEvent, tabId: string, targetSection: 'upper' | 'lower') {
    ...
    if (result?.type === 'section-change') {
      onSectionChange?.(result.tabId!, result.targetSection);
    } else if (result?.type === 'reorder' && result.reordered) {
      if (targetSection === 'upper') {
        tabs = result.reordered;
        onReorder?.(result.reordered);
      } else {
        lowerTabs = result.reordered;
        onLowerReorder?.(result.reordered);
      }
    }
    resetDrag();
  }
  ...
  function handleDropZone(e: DragEvent) {
    e.preventDefault();
    if (!draggedTabId || dragSource === 'lower') {
      resetDrag();
      return;
    }
    onSectionChange?.(draggedTabId, 'lower');
    resetDrag();
  }
  function handleUpperDropZone(e: DragEvent) {
    e.preventDefault();
    if (!draggedTabId || dragSource === 'upper') {
      resetDrag();
      return;
    }
    onSectionChange?.(draggedTabId, 'upper');
    resetDrag();
  }
  function handleDropZoneDragOver(e: DragEvent) {
    e.preventDefault();
    if (!draggedTabId || dragSource === 'lower') return;
    if (e.dataTransfer) e.dataTransfer.dropEffect = 'move';
    dropZoneActive = true;
  }
  function handleUpperDropZoneDragOver(e: DragEvent) {
    e.preventDefault();
    if (!draggedTabId || dragSource === 'upper') return;
    if (e.dataTransfer) e.dataTransfer.dropEffect = 'move';
    upperDropZoneActive = true;
  }
  function handleDropZoneDragLeave() {
    dropZoneActive = false;
  }
  function handleUpperDropZoneDragLeave() {
...
>
  ...
  <div class="tabs-main">
    <div
      class="tabs-section"
      role="list"
      on:dragover={handleUpperDropZoneDragOver}
      on:dragleave={handleUpperDropZoneDragLeave}
      on:drop={handleUpperDropZone}
      class:drop-zone-active={upperDropZoneActive}
    >
      {#each tabs as tab (tab.id)}
        <VerticalTabButton
          {tab}
          active={activeTab === tab.id}
          dragOver={dropTargetId === tab.id}
          onSelect={selectTab}
          onKeydown={handleKeydown}
          onDragStart={(e) => handleDragStart(e, tab.id, 'upper')}
          onDragOver={(e) => handleDragOver(e, tab.id)}
          onDragLeave={handleDragLeave}
          onDrop={(e) => handleDrop(e, tab.id, 'upper')}
      ...
      <div class="tabs-section tabs-lower">
        {#each lowerTabs as tab (tab.id)}
          <VerticalTabButton
            ...
            onSelect={selectTab}
            onKeydown={handleKeydown}
            onDragStart={(e) => handleDragStart(e, tab.id, 'lower')}
            onDragOver={(e) => handleDragOver(e, tab.id)}
            onDragLeave={handleDragLeave}
            onDrop={(e) => handleDrop(e, tab.id, 'lower')}
            onDragEnd={handleDragEnd}
          />
        {/each}
      </div>
...
<style>
  ...
  .section-divider.drop-zone-active {
