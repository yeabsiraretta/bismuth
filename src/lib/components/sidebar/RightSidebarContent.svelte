...
{:else if activeTab === 'outgoing'}
  {#if activeNotePath}
    ...
    />
  {:else}
    <EmptyState icon="external-link" title="No note open" description="Open a note to see outgoing links" />
  {/if}
...
{:else if activeTab === 'dataview'}
  <AsyncFeature
    featureId="dataview"
    loader={() => import('@/features/dataview/components/DataviewPanel.svelte').then(m => ({ default: m.default }))}
  />
{/if}
