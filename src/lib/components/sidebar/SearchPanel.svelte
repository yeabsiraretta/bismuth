<script lang="ts">
  ...
  import { notes, setActiveNote } from '@/stores/vault/vault';
  ...
  const filterOptions = ['tag', 'recent', 'title-only'];
  ...
  function handleInput() {
    ...
    debounceTimer = setTimeout(() => {
      results = doSearch(query, $notes, activeFilters);
  ...
  function toggleFilter(filter: string) {
    ...
    if (query.trim()) { results = doSearch(query, $notes, activeFilters); }
  }

  async function openResult(note: Note) {
    try { const fullNote = await getNote(note.path); setActiveNote(fullNote); }
    catch { setActiveNote(note); }
  }
...
<div class="search-panel">
  <div class="search-input-area">
    <div class="search-input-wrapper">
      ...
      />
      {#if query}
        <button class="clear-btn" on:click={() => { query = ''; results = []; }} aria-label="Clear search">
          <Icon name="x" size={12} />
        </button>
      {/if}
    </div>
  ...
  <div class="search-results">
    ...
    {:else if query && results.length > 0}
      ...
      {#each results as note (note.path)}
        <button class="result-item" on:click={() => openResult(note)}>
          <div class="result-title">
            <Icon name="file-text" size={14} />
            <span>{note.title}</span>
          </div>
          {#if note.content && query}
            <p class="result-snippet">{@html getSnippet(note.content, query).replace(
    ...
    {:else if !query && recentSearches.length > 0}
      <div class="recent-section">
        <div class="recent-header">
          ...
          <button class="clear-recent" on:click={clearRecent}>Clear</button>
        </div>
        {#each recentSearches as term}
          <button class="recent-item" on:click={() => useRecent(term)}>
