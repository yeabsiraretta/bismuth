<script lang="ts">
  ...
  onMount(() => {
    ...
    renderer.text = (token) => {
      ...
      const replaced = t.replace(/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g, (_, target, alias) => {
...
<style>
  ...
  .markdown-preview :global(mark) {
    background: var(--text-highlight-bg, rgba(255, 208, 0, 0.4));
    border-radius: 2px;
    padding: 1px 0;
  }
  .markdown-preview :global(u) {
    text-decoration: underline;
  }
  .markdown-preview :global(sup) {
    vertical-align: super;
    font-size: 0.75em;
  }
  .markdown-preview :global(sub) {
    vertical-align: sub;
    font-size: 0.75em;
  }
  ...
  .markdown-preview :global(.md-code-spacer) {
    flex: 1;
  }
  ...
  .markdown-preview :global(a.md-wikilink) {
    color: var(--interactive-accent);
    text-decoration: none;
  }
  .markdown-preview :global(a.md-wikilink:hover) {
    text-decoration: underline;
  }
