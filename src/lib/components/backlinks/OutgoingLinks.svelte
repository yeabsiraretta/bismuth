<script lang="ts">
  ...
  async function loadOutgoingLinks() {
    try {
      loading = true;
      error = null;
      const data = await invoke<OutgoingLinksData>('get_outgoing_links', { noteId });
      outgoingData = data;
    } catch (err) {
      error = err instanceof Error ? err.message : 'Failed to load outgoing links';
      console.error('Error loading outgoing links:', err);
    } finally {
      loading = false;
    }
  }

  function openNote(targetId: string, line?: number) {
    window.dispatchEvent(
      new CustomEvent('open-note', {
        detail: { id: targetId, line },
      })
    );
  }
  ...
  async function createLinkFromMention(
    ...
    try {
      await invoke('create_link_from_unlinked_mention', {
        sourceNoteId: noteId,
        targetNoteId: targetNote.id,
        lineNumber: mention.lineNumber,
        text: mention.potentialTargetName,
      });
