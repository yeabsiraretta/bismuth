...
function saveJson<T>(key: string, val: T): void {
  try {
    ...
    log.warn('Failed to persist home tab data', { key, error: String(e) });
  }
}

export const homeSettings = writable<HomeTabSettings>(
  loadJson(SETTINGS_KEY, DEFAULT_HOME_SETTINGS)
);
...
export const recentNotes = derived([recentFilePaths, notes], ([$paths, $notes]) => {
  const noteMap = new Map($notes.map((n) => [n.path, n]));
  return $paths.map((p) => noteMap.get(p)).filter((n): n is NonNullable<typeof n> => !!n);
});
