<script lang="ts">
  ...
  $: {
    ...
    if (query.trim() || activeFilter) {
      results = searchFiles(
        $notes,
        activeFilter ? detectFilter(query).cleanQuery : query,
        activeFilter
      );
      showResults = true;
    } else {
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    if (e.key === 'Enter' && selectedIdx >= 0 && results[selectedIdx]) {
      e.preventDefault();
      handleOpen(results[selectedIdx].path);
    }
    if (e.key === 'Escape') {
      query = '';
      activeFilter = null;
      ...
      <div class="search-wrapper">
        <div class="search-bar">
          ...
          <input
            ...
            on:focus={() => {
              if (query || activeFilter) showResults = true;
            }}
            type="text"
            class="search-input"
            placeholder={activeFilter
              ? `Search ${activeFilter.label} files...`
              : 'Search files... (type filter name + Tab)'}
          ...
          <ul class="search-results" role="listbox">
            {#each results as result, i (result.path)}
              <li
                ...
                on:click={() => handleOpen(result.path)}
                on:keydown={(e) => e.key === 'Enter' && handleOpen(result.path)}
                tabindex="-1"
              >
