...
export function noteAtPoint(cx: number, cy: number, notes: MidiNote[], scrollX: number, scrollY: number): MidiNote | null {
...
export function isResizeHandle(cx: number, note: MidiNote, scrollX: number, scrollY: number): boolean {
...
export function drawGrid(c: CanvasRenderingContext2D, w: number, h: number, scrollX: number, scrollY: number, div: GridDivision) {
  c.save();
  for (let i = 0; i < TOTAL_PITCHES; i++) {
    const y = (TOTAL_PITCHES - 1 - i) * NOTE_HEIGHT - scrollY;
    if (y < -NOTE_HEIGHT || y > h) continue;
    c.fillStyle = BLACK_KEYS.has((MIDI_LOW + i) % 12) ? 'rgba(0,0,0,0.25)' : 'rgba(255,255,255,0.03)';
...
export function drawNotes(c: CanvasRenderingContext2D, notes: MidiNote[], canvasWidth: number, scrollX: number, scrollY: number, accentColor: string) {
  c.save();
