<script lang="ts">
  ...
  import {
    PIANO_KEY_WIDTH, NOTE_HEIGHT, TOTAL_PITCHES, PIXELS_PER_BEAT,
    TICKS_PER_BEAT, divisionToTicks, quantizeTick,
    noteAtPoint, isResizeHandle, yToPitch, xToTick,
    drawGrid, drawPianoKeys, drawNotes,
    type GridDivision,
  ...
  function onPointerDown(e: MouseEvent) {
    ...
    if (hit) {
      if (isResizeHandle(cx, hit, scrollX, scrollY)) {
        dragMode = 'resize'; dragNote = { ...hit }; dragOrigDur = hit.durationTicks;
      } else {
        dragMode = 'move'; dragNote = { ...hit }; dragOrigTick = hit.startTick;
      }
      dragStartX = cx; dragStartY = cy;
    } else if (cx > PIANO_KEY_WIDTH) {
      dragMode = 'new'; dragStartX = cx; dragStartY = cy;
    }
  }
  ...
  function onPointerMove(e: MouseEvent) {
    ...
    if (dragMode === 'move' && dragNote) {
      ...
      notes = notes.map((n) => n.startTick === dragNote!.startTick && n.pitch === dragNote!.pitch
        ? { ...n, startTick: newTick, pitch: newPitch } : n);
      draw();
...
<style>
  .piano-roll { display:flex; flex-direction:column; flex:1 1 0; overflow:hidden; background:var(--background-primary,#181825); --color-accent:#6366f1; }
  .toolbar { display:flex; align-items:center; gap:var(--spacing-xs,4px); padding:4px 8px; background:var(--background-secondary,#1e1e2e); border-bottom:1px solid var(--background-modifier-border,#313244); flex-shrink:0; }
  .grid-btn { background:var(--background-modifier-hover,#313244); border:1px solid var(--background-modifier-border,#45475a); border-radius:var(--radius-s,4px); color:var(--text-muted,#a6adc8); cursor:pointer; font-size:10px; font-family:var(--font-mono); padding:2px 8px; transition:color 0.1s,background 0.1s; }
  .grid-btn.active { background:var(--interactive-accent,#6366f1); border-color:var(--interactive-accent,#6366f1); color:#fff; }
  .toolbar-hint { margin-left:auto; font-size:10px; color:var(--text-faint,#6c7086); }
  .canvas-wrap { flex:1; overflow:auto; cursor:crosshair; }
