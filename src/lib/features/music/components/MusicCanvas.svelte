  ...
  {:else}
      ...
      <div class="ruler-scroll">
        <div class="ruler" style="width:{timelineWidth}px" role="presentation">
          {#each barNumbers as bar}
            <div class="ruler-mark" style="width:{BAR_WIDTH}px" aria-label="Bar {bar}">
    ...
    <div class="lanes-container">
      {#each tracks as track (track.id)}
        ...
        >
          <!-- Label -->
          <div class="label-col" style="border-left: 3px solid {clipColor(track)}">
...
<style>
  ...
  .export-area {
    ...
    border-left: 1px solid var(--background-modifier-border, #313244);
  }
  .lanes-container {
    flex: 1;
    overflow: auto;
  ...
  .add-track-btn:hover {
    color: var(--text-normal, #cdd6f4);
    border-color: var(--text-muted, #a6adc8);
  }
</style>
