...
function getChordDb(instrument: ChordInstrument): Record<string, Record<string, number[][]>> {
  switch (instrument) {
    ...
    case 'mandolin':
      return UKULELE_CHORDS; // mandolin shares open tuning shapes
    default:
...
export function renderChordDiagramSvg(
  ...
  // Title
  lines.push(
    `<text x="${width / 2}" y="${titleHeight}" text-anchor="middle" font-size="13" font-weight="600" fill="currentColor">${escapeXml(chordName)}</text>`
  );
  ...
  if (fingering.baseFret > 1) {
    lines.push(
      `<text x="${gridLeft - 8}" y="${gridTop + fretSpacing * 0.6}" text-anchor="end" font-size="10" fill="currentColor">${fingering.baseFret}fr</text>`
    );
  } else {
    // Nut (thick top line)
    lines.push(
      `<line x1="${gridLeft}" y1="${gridTop}" x2="${gridLeft + gridWidth}" y2="${gridTop}" stroke="currentColor" stroke-width="3"/>`
    );
  }
  ...
  for (let f = 0; f <= fretCount; f++) {
    ...
    lines.push(
      `<line x1="${gridLeft}" y1="${y}" x2="${gridLeft + gridWidth}" y2="${y}" stroke="currentColor" stroke-opacity="0.3" stroke-width="1"/>`
    );
  }
  ...
  for (let s = 0; s < stringCount; s++) {
    ...
    lines.push(
      `<line x1="${x}" y1="${gridTop}" x2="${x}" y2="${gridTop + gridHeight}" stroke="currentColor" stroke-opacity="0.4" stroke-width="1"/>`
    );
  }
  ...
  for (let s = 0; s < stringCount && s < fingering.frets.length; s++) {
    ...
    if (fret === -1) {
      // Muted
      lines.push(
        `<text x="${x}" y="${gridTop - 4}" text-anchor="middle" font-size="10" fill="currentColor">×</text>`
      );
    } else if (fret === 0) {
      // Open
      lines.push(
        `<circle cx="${x}" cy="${gridTop - 6}" r="${dotRadius * 0.7}" fill="none" stroke="currentColor" stroke-width="1.5"/>`
      );
    } else {
      // Fretted
