      ...
      <div class="recorder-controls">
        ...
        <div class="control-row">
          <button
            ...
            title={$recordingState === 'idle' ? 'Start recording' : $recordingState === 'recording' ? 'Pause' : 'Resume'}
          >
            <Icon name={$recordingState === 'recording' ? 'pause' : 'mic'} size={20} />
          </button>

          {#if $recordingState !== 'idle'}
            <button class="stop-btn" on:click={handleStop} title="Stop and save" disabled={$voiceLoading}>
          ...
          {#each $recordings as rec}
            <div class="recording-item">
              <div class="rec-info">
                ...
                <span class="rec-meta">{formatDuration(rec.duration)} - {formatDate(rec.createdAt)}</span>
              </div>
              <div class="rec-actions">
                {#if $activeNote}
                  <button class="rec-btn" title="Attach to note" on:click={() => handleAttach(rec.id)}>
                    <Icon name="link" size={12} />
                  </button>
                {/if}
                <button class="rec-btn danger" title="Delete" on:click={() => removeRecording(rec.id)}>
...
<style>
  ...
  .panel-body { padding: 12px; display: flex; flex-direction: column; gap: 16px; }
  .unsupported { text-align: center; padding: 24px; color: var(--text-muted); }
  .unsupported p { margin: 8px 0 0; font-size: 12px; }
  .recorder-controls { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 16px 0; }
  .timer { font-size: 28px; font-weight: 600; font-variant-numeric: tabular-nums; color: var(--text-primary); }
  ...
  .record-btn.recording { background: var(--error-color, #ef4444); animation: pulse 1.5s infinite; }
  .record-btn.paused { background: var(--warning-color, #f59e0b); }
  .stop-btn {
    width: 36px; height: 36px; border-radius: 50%; border: none;
    background: var(--surface-color, #313244); color: var(--text-primary);
    display: flex; align-items: center; justify-content: center; cursor: pointer;
  }
  .stop-btn:hover { background: var(--hover-bg); }
  .recording-label { font-size: 11px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px; }
  .recordings-list { display: flex; flex-direction: column; gap: 4px; }
  .list-label { font-size: 11px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; padding: 4px 0; }
  .recording-item {
    display: flex; align-items: center; padding: 8px; border-radius: 4px;
    border: 1px solid var(--border-color);
  }
  .recording-item:hover { background: var(--hover-bg); }
  ...
  .rec-btn:hover { background: var(--hover-bg); color: var(--text-primary); }
  .rec-btn.danger:hover { color: var(--error-color, #f87171); }
