<script lang="ts">
  ...
  import {
    reviewConfig, reviewGroups, reviewRandomNote,
    reviewLoading, reviewTotalEntries, refreshRandomNote,
...
<div class="review-panel" role="tabpanel" aria-label="Journal Review">
  <PanelHeader icon="history" title="On This Day">
    <svelte:fragment slot="actions">
      <ActionButton icon="refresh-cw" title="Refresh" on:click={() => window.dispatchEvent(new CustomEvent('journal-review-refresh'))} />
    </svelte:fragment>
  </PanelHeader>
      ...
      {#if config.showRandomNote && randomNote && config.randomNotePosition === 'top'}
        <div class="random-section">
          ...
          <button class="entry-card" on:click={() => handleOpenNote(randomNote.path)}>
            ...
            {/if}
            <span class="entry-date">{formatDate(randomNote.createdDate)} - {randomNote.timeAgo}</span>
...
<style>
  .review-panel { display: flex; flex-direction: column; height: 100%; }
  .panel-body { padding: 12px; display: flex; flex-direction: column; gap: 16px; overflow-y: auto; flex: 1; }
  ...
  .review-group { display: flex; flex-direction: column; gap: 8px; }
  .group-header {
    display: flex; align-items: center; gap: 8px;
    padding: 6px 0; border-bottom: 1px solid var(--border-color);
  }
  .group-label { font-size: 13px; font-weight: 600; color: var(--text-primary); }
  .group-date { font-size: 11px; color: var(--text-muted); flex: 1; }
  .group-count {
    font-size: 10px; background: var(--accent-color, #6366f1); color: white;
  ...
  .entry-title { font-size: 13px; font-weight: 600; color: var(--text-primary); }
  ...
  .entry-preview.callout {
    ...
    background: var(--background-primary); border-radius: 0 4px 4px 0;
    padding: 8px 10px;
  }
  ...
  .random-section {
    ...
    padding: 8px; border: 1px dashed var(--border-color);
  ...
  .shuffle-btn {
    margin-left: auto; padding: 2px 6px; border: none;
    background: none; color: var(--text-muted); cursor: pointer;
    border-radius: 4px;
  }
  .shuffle-btn:hover { background: var(--background-modifier-hover); }
</style>
