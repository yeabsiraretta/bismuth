...
export const importHistory = writable<Array<ImportResult & { source: ImportSource; timestamp: string }>>([]);
...
export const progressLabel = derived(importProgress, ($p) => {
  if (!$p) return '';
  const phase = $p.phase === 'reading' ? 'Reading' : $p.phase === 'converting' ? 'Converting' : 'Writing';
...
export async function executeImport(): Promise<ImportResult | null> {
  ...
  importOptions.subscribe(v => { opts = v; })();
  ...
  try {
    ...
    if (result.imported > 0 || result.failed > 0) {
      importHistory.update(h => [
        { ...result, source: opts.source, timestamp: new Date().toISOString() },
        ...h,
      ].slice(0, 20));
    }
