...
export const filteredPMTasks = derived(
  ...
  ([$tasks, $filter, $sort]) => {
    let result = $tasks.filter((t) => {
      ...
      if ($filter.assignee?.length && !t.assignees.some((a) => $filter.assignee!.includes(a)))
        return false;
...
export async function loadProjectTasks(projectFolder: string): Promise<void> {
  ...
  try {
    ...
    const taskNotes = allNotes.filter(
      (n) => n.path.startsWith(projectFolder) && n.frontmatter?.['pm-task'] === true
    );
