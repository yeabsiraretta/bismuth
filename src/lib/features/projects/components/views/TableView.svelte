...
<div class="table-view">
  ...
  {#if hasSelection}
    <div class="bulk-bar">
      ...
      <button class="bulk-btn" on:click={() => bulkSetStatus([...$selectedTaskIds], 'done')}
        >Done</button
      >
      <button class="bulk-btn" on:click={() => bulkSetPriority([...$selectedTaskIds], 'high')}
        >High</button
      >
      <button class="bulk-btn" on:click={() => bulkArchive([...$selectedTaskIds], true)}
        >Archive</button
      >
      <button class="bulk-btn danger" on:click={() => bulkDelete([...$selectedTaskIds])}
        >Delete</button
      >
      ...
      <thead>
        <tr>
          <th class="col-check"
            ><input
              type="checkbox"
              checked={$filteredPMTasks.length > 0 &&
                $filteredPMTasks.every((t) => $selectedTaskIds.has(t.id))}
              on:change={toggleSelectAll}
            /></th
          >
          <th class="col-status" on:click={() => toggleSort('status')}
            >Status {#if sortIcon('status')}<Icon name={sortIcon('status')} size={10} />{/if}</th
          >
          <th class="col-title" on:click={() => toggleSort('title')}
            >Title {#if sortIcon('title')}<Icon name={sortIcon('title')} size={10} />{/if}</th
          >
          <th class="col-priority" on:click={() => toggleSort('priority')}
            >Priority {#if sortIcon('priority')}<Icon
                name={sortIcon('priority')}
                size={10}
              />{/if}</th
          >
          <th class="col-assignee" on:click={() => toggleSort('assignee')}>Assignee</th>
          <th class="col-due" on:click={() => toggleSort('dueDate')}
            >Due {#if sortIcon('dueDate')}<Icon name={sortIcon('dueDate')} size={10} />{/if}</th
          >
          ...
          <tr class:selected={$selectedTaskIds.has(task.id)} class:done={task.status === 'done'}>
            ...
            <td class="col-status">
              <button
                ...
                on:click={() => cycleStatus(task)}
                title="Click to cycle status"
              >
                <Icon
                  name={DEFAULT_STATUSES.find((s) => s.id === task.status)?.icon ?? 'circle'}
                  size={12}
                />
                <span
                  >{DEFAULT_STATUSES.find((s) => s.id === task.status)?.label ?? task.status}</span
                >
              </button>
            </td>
            <td class="col-title">
              {#if editingId === task.id}
                <input
                  ...
                  on:keydown={(e) => e.key === 'Enter' && finishEdit(task)}
                />
              {:else}
...
<style>
  ...
  td {
    padding: var(--spacing-xs) var(--spacing-s);
    border-bottom: 1px solid var(--background-modifier-border, rgba(0, 0, 0, 0.05));
