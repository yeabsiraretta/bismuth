...
<div class="clipper-panel">
  ...
  <div class="clipper-actions">
    <button
      ...
      on:click={clipFromClipboard}
      disabled={$isClipping}
      title="Clip from clipboard (single URL)"
    >
      <Icon name="clipboard" size={14} />
      <span>Clip URL</span>
    </button>
    <button
      class="action-btn"
      on:click={clipBatchFromClipboard}
      disabled={$isClipping}
      title="Batch clip URLs from clipboard"
    >
        ...
        <div class="clip-item {getStatusClass(item.status)}">
          ...
          <div class="clip-info">
            {#if item.notePath}
              <button
                class="clip-title"
                on:click={() => openNote(item.notePath || '')}
                title="Open note"
              >
                {item.title}
              </button>
...
<style>
  ...
  .status-saved .clip-status {
    color: var(--interactive-accent, #10b981);
  }
  .status-error .clip-status {
    color: var(--text-error, #ef4444);
  }
  .status-pending .clip-status {
    color: var(--text-muted);
  }
  .clear-btn {
    display: block;
