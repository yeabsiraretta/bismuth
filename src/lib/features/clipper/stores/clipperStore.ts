...
function buildContentVars(
  ...
  const CONTENT_VARS: Record<ContentType, () => Record<string, string>> = {
    youtube: () => ({
      videoURL: url,
      videoId: id ?? '',
      videoTitle: `YouTube Video ${id ?? ''}`,
      videoPlayer: id ? generateVideoEmbed('youtube', id) : '',
      videoDescription: '',
      channelName: '',
      channelURL: '',
      channelId: '',
      videoThumbnail: id ? `https://img.youtube.com/vi/${id}/maxresdefault.jpg` : '',
      videoPublishDate: base['date'] ?? '',
      videoChapters: '',
      videoViewsCount: '',
    }),
    'youtube-channel': () => ({
      ...
      channelVideosCount: '',
      channelVideosURL: `${url}/videos`,
      channelShortsURL: `${url}/shorts`,
    }),
    twitter: () => ({
      tweetURL: url,
      tweetAuthorName: '',
