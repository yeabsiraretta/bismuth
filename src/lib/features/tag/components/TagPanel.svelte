<script lang="ts">
  ...
  function renameTag(name: string) {
    renameTagOp(name, closeContextMenu);
  }
  function mergeTag(name: string) {
    mergeTagOp(name, closeContextMenu);
  }
  function createTagPage(name: string) {
    createTagPageOp(name, onOpenNote, closeContextMenu);
  }
  function openRandomNoteWithTag(name: string) {
    openRandomOp(name, onOpenNote, closeContextMenu);
  }
  function hideTag(name: string) {
    hideTagOp(name, closeContextMenu);
  }
  function unhideTag(name: string) {
    unhideTagOp(name);
  }
</script>
            ...
            <div class="tag-children">
              {#each tag.children as childTag}
                ...
                >
                  ...
                  <button
                    class="tag-name-btn"
                    on:click={() => selectTag(childTag.name)}
                    title="Filter by {childTag.name}"
                  >
