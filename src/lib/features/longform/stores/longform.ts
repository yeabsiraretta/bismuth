...
import { discoverProjects, createDraft, listDrafts, updateSceneStatus } from '../services/longform';
...
function defaultPresets(): CompilePreset[] {
  return [
    ...
    },
    {
      ...
      include_scene_titles: true,
      format: 'markdown',
      scene_filter: ['drafting', 'revising', 'complete'],
    },
  ];
}
    ...
    activeProject.update((p) => {
      ...
      return {
        ...
        scenes: p.scenes.map((s) =>
          s.path === (scene as Scene).path ? { ...s, draft_count: s.draft_count + 1 } : s
        ),
      };
    });
...
export async function changeSceneStatus(status: SceneStatus): Promise<void> {
  ...
  try {
    ...
    activeProject.update((p) => {
      ...
      return {
        ...p,
        scenes: p.scenes.map((s) => (s.path === (scene as Scene).path ? { ...s, status } : s)),
      };
    });
...
compilePresets.subscribe((presets) => {
  try {
    localStorage.setItem('bismuth-compile-presets', JSON.stringify(presets));
  } catch (e) {
    log.warn('Failed to persist compile presets to localStorage', { error: String(e) });
  }
});
