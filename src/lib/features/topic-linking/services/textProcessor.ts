...
const STOPWORDS = new Set([
  ...
  'while', 'who', 'whom', 'why', 'you', 'your', 'one', 'two', 'first',
  'also', 'back', 'get', 'go', 'see', 'come', 'take', 'know', 'way',
  'well', 'even', 'still', 'think', 'say',
]);
...
export function stem(word: string): string {
  ...
  const suffixes = ['ational', 'tional', 'alize', 'ation', 'ness', 'ment', 'ful', 'ous', 'ive', 'able', 'ible', 'ity', 'ist', 'ism', 'ly'];
...
export function sampleTokens(
  ...
  if (config.randomise) {
    ...
    return indices.slice(0, count).sort((a, b) => a - b).map(i => tokens[i]);
  }
...
export function matchesTags(docTags: string[], tagPattern: string): boolean {
  ...
  const patterns = tagPattern.trim().split(/\s+/).map(t => t.replace(/^#/, '').toLowerCase());
