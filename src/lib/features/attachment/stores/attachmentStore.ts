...
import type { AttachmentConfig, AttachmentOverride, OriginalNameEntry } from '../types';
...
function persist(key: string, value: unknown): void {
  try {
    ...
    log.warn('attachmentStore: persist failed', { key, error: String(e) });
  }
}

const configInternal = writable<AttachmentConfig>(loadJson(CONFIG_KEY, DEFAULT_ATTACHMENT_CONFIG));
