...
import { listCourses, saveCourse, deleteCourse, listTopics, saveTopic } from '../services/courseService';
...
export const activeCourse = derived(_store, s =>
  s.courses.find(c => c.id === s.activeCourseId) ?? null
);
...
export async function saveCourseAction(course: Course): Promise<void> {
  ...
  _store.update(s => {
    ...
    const courses = idx >= 0
      ? s.courses.map((c, i) => i === idx ? saved : c)
      : [...s.courses, saved];
    return { ...s, courses };
  });
}
...
export async function markTopicMastered(courseId: string, notePath: string, mastered: boolean): Promise<void> {
  ...
  const topic = topics.find(t => t.notePath === notePath) ?? {
    notePath, mastered: false, lastReviewed: null, cardCount: 0,
  };
