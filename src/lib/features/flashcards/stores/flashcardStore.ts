...
export async function syncToAnki(): Promise<SyncResult | null> {
  ...
  if (state.connectionStatus !== 'connected') {
    ...
    if (get(_store).connectionStatus !== 'connected') {
      _store.update((s) => ({
        ...s,
        error: 'Anki is not reachable. Make sure Anki is running with AnkiConnect installed.',
      }));
      return null;
    }
  }
  ...
  try {
    ...
    log.info('Flashcard sync complete', {
...
export async function importCardsFromAnki(): Promise<number> {
  ...
  try {
    ...
    const newCards = imported.filter(
      (c) => c.ankiNoteId !== null && !existingNoteIds.has(c.ankiNoteId)
    );
    _store.update((s) => ({
      ...s,
      scanning: false,
      scannedCards: [...s.scannedCards, ...newCards],
    }));
