<script lang="ts">
  ...
  import {
    scanActiveNote,
    scannedCards, isScanning, flashcardError, cardCount,
  } from '../stores/flashcardStore';
  import { getDueCards, deserializeRecords, serializeRecords, gradeCard } from '../services/scheduler';
  ...
  function endSession() { viewMode = 'cards'; }

  function handleRescan() {
    const unsub = activeNote.subscribe(n => { if (n) scanActiveNote(n.path, n.content); });
    unsub();
  }
</script>
  ...
  {#if currentPath && viewMode === 'cards'}
    <div class="panel-stats">
      ...
      <div class="stat-actions">
        <button class="action-btn secondary" on:click={handleRescan} disabled={$isScanning}>Rescan</button>
      </div>
    </div>
...
<style>
  .flashcard-panel { display: flex; flex-direction: column; height: 100%; overflow: hidden; background: var(--background-secondary); }
  .panel-header { display: flex; align-items: center; gap: var(--spacing-s); padding: var(--spacing-s) var(--spacing-m); border-bottom: 1px solid var(--border-color); }
  ...
  .note-name { font-size: var(--font-smallest); color: var(--text-faint); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  ...
  .error-banner { padding: var(--spacing-xs) var(--spacing-m); background: var(--background-modifier-error); color: var(--text-error); font-size: var(--font-smallest); }
  .panel-stats { display: flex; align-items: center; gap: var(--spacing-m); padding: var(--spacing-s) var(--spacing-m); border-bottom: 1px solid var(--border-color); }
  .stat { display: flex; flex-direction: column; align-items: center; }
  .stat-value { font-size: var(--font-normal); font-weight: var(--font-bold); color: var(--text-normal); }
  .stat-value.due { color: var(--interactive-accent); }
  ...
  .action-btn.secondary:not(:disabled):hover { background: var(--background-modifier-active-hover); }
  .study-btn { margin: 0 var(--spacing-m) var(--spacing-s); padding: var(--spacing-s); background: var(--interactive-accent); color: var(--text-on-accent); border: none; border-radius: var(--radius-m); font-size: var(--font-smaller); font-weight: var(--font-semibold); cursor: pointer; }
  ...
  .card-question :global(mark) { background: rgba(245, 158, 11, 0.2); color: inherit; border-radius: 2px; padding: 0 3px; border-bottom: 2px dashed rgba(245, 158, 11, 0.5); }
  .card-question :global(.cloze-blank) { background: var(--background-modifier-border); color: transparent; border-radius: 3px; padding: 0 12px; user-select: none; }
  ...
  .reveal-btn { margin-top: auto; padding: var(--spacing-s); background: var(--interactive-accent); color: var(--text-on-accent); border: none; border-radius: var(--radius-m); font-size: var(--font-smaller); font-weight: var(--font-semibold); cursor: pointer; transition: filter 0.12s; }
