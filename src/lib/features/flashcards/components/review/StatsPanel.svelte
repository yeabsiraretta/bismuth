<script lang="ts">
  ...
  $: maturedPct =
    stats.totalCards > 0 ? Math.round((stats.maturedCards / stats.totalCards) * 100) : 0;
  $: youngPct = stats.totalCards > 0 ? Math.round((stats.youngCards / stats.totalCards) * 100) : 0;
  $: newPct = stats.totalCards > 0 ? Math.round((stats.newCards / stats.totalCards) * 100) : 0;
</script>
...
<div class="stats-panel">
  ...
  <div class="maturity-bar">
    {#if stats.totalCards > 0}
      <div
        class="bar-segment matured"
        style="width: {maturedPct}%"
        title="{stats.maturedCards} matured ({maturedPct}%)"
      ></div>
      <div
        class="bar-segment young"
        style="width: {youngPct}%"
        title="{stats.youngCards} young ({youngPct}%)"
      ></div>
      <div
        class="bar-segment new"
        style="width: {newPct}%"
        title="{stats.newCards} new ({newPct}%)"
      ></div>
    {/if}
  </div>
