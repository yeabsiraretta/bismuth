<script lang="ts">
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    switch (e.key) {
      ...
      case 'Escape':
        e.preventDefault();
        presenting ? stopMarpPresentation() : closeMarpPreview();
        break;
      case 'Home':
        e.preventDefault();
        goToMarpSlide(0);
  ...
  <div class="marp-preview" class:fullscreen={presenting}>
    {#if !presenting}
      <div class="marp-toolbar">
        ...
        <div class="marp-nav">
          <button class="marp-btn" on:click={prevMarpSlide} disabled={idx === 0} title="Previous"
            >◀</button
          >
          <button class="marp-btn" on:click={nextMarpSlide} disabled={idx >= total - 1} title="Next"
            >▶</button
          >
        </div>
    ...
    <div class="slide-viewport" bind:this={slideContainer}>
      {#if slide}
        <div
          ...
          style={pres.globalDirectives.backgroundColor
            ? `background-color: ${pres.globalDirectives.backgroundColor};`
            : ''}
        >
