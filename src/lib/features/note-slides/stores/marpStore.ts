...
function loadConfig(): MarpConfig {
  ...
  } catch { return { ...DEFAULT_MARP_CONFIG }; }
}
...
export const currentSlide = derived(_state, s =>
  s.presentation?.slides[s.activeSlide] ?? null,
);
...
export function openMarpPreview(markdown: string, notePath: string): void {
  ...
  if (!pres) {
    _state.update(s => ({ ...s, error: 'Not a valid Marp presentation (missing marp: true in frontmatter)' }));
