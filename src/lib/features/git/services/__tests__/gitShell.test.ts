...
import { GitError, runGitStatus, runGitAdd, runGitCommit, runGitLog, runGitBranch } from '../gitShell';
...
function mockCommand(stdout: string, stderr = '', code = 0) {
  ...
  vi.mocked(Command.create).mockReturnValue({ execute } as unknown as ReturnType<typeof Command.create>);
  return execute;
}
...
describe('gitShell', () => {
  ...
  it('runGitAdd calls git add with files', async () => {
    ...
    expect(Command.create).toHaveBeenCalledWith('git', ['-C', '/vault', 'add', '--', 'foo.ts', 'bar.ts']);
  });
  ...
  it('runGitCommit calls git commit -m', async () => {
    ...
    expect(Command.create).toHaveBeenCalledWith('git', ['-C', '/vault', 'commit', '-m', 'feat: add thing']);
  });
  ...
  it('runGitLog calls git log --max-count', async () => {
    ...
    expect(Command.create).toHaveBeenCalledWith('git', ['-C', '/vault', 'log', '--max-count=5', '--oneline']);
  ...
  it('runGitBranch calls git branch --show-current', async () => {
    ...
    expect(Command.create).toHaveBeenCalledWith('git', ['-C', '/vault', 'branch', '--show-current']);
