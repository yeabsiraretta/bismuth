<script lang="ts">
  ...
  function openNote(path: string) { onOpenNote ? onOpenNote(path) : navOpenNote(path); }
  ...
  {:else}
    ...
    <div class="lookup-container">
      <div class="lookup-input">
        ...
        />
        <button class="lookup-btn" on:click={handleLookup} disabled={isSearching} title="Search" aria-label="Search">
...
<style>
  ...
  .icon-btn { display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; background: none; border: none; border-radius: var(--radius-s); color: var(--text-muted); cursor: pointer; transition: all 0.15s ease; }
  ...
  .loading-state p { font-size: var(--font-ui-small); font-weight: var(--font-medium); margin: 0; }
  .connections-list { display: flex; flex-direction: column; gap: var(--spacing-s); flex: 1; overflow-y: auto; }
  .section-label { font-size: var(--font-smallest); font-weight: var(--font-semibold); text-transform: uppercase; letter-spacing: 0.04em; color: var(--text-faint); padding: 4px 0; }
  .tab-bar { display: flex; gap: 0; margin-bottom: var(--spacing-m); border-bottom: 1px solid var(--border-color); }
  .tab-btn { flex: 1; padding: var(--spacing-xs) var(--spacing-s); background: none; border: none; border-bottom: 2px solid transparent; color: var(--text-muted); font-size: var(--font-smallest); font-weight: var(--font-medium); cursor: pointer; transition: all 0.15s ease; }
