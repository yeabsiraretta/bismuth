...
<div class="pg-view">
  <div class="pg-header">
    ...
    <div class="pg-add">
      <input
        class="pg-input"
        bind:value={newPlotlineName}
        placeholder="Add plotline…"
        on:keydown={(e) => e.key === 'Enter' && handleAddPlotline()}
      />
          ...
          {#each scenes as scene (scene.id)}
            <tr>
              <td class="pg-row-header">
                <span
                  class="pg-status-dot"
                  style="background: {STATUS_COLORS[scene.status] ?? '#666'}"
                ></span>
                ...
                <td class="pg-cell" on:dblclick={() => startEdit(key, text)}>
                  {#if editingCellKey === key}
                    <textarea
                      class="pg-cell-edit"
                      bind:value={editText}
                      on:blur={() => saveEdit(scene.id, pl.id)}
                      on:keydown={(e) => e.key === 'Escape' && (editingCellKey = null)}
                    ></textarea>
                  {:else}
...
<style>
  ...
  .pg-col-header:hover .pg-col-delete {
    opacity: 1;
  }
  .pg-row-header {
    position: sticky;
