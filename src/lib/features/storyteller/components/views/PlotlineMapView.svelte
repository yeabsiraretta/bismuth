...
<div class="pm-view">
  ...
  {#if viewMode === 'subway'}
    <div
      ...
      role="application"
      aria-label="Plotline subway map"
    >
      <svg
        width={svgWidth}
        height={lanes.length * 50 + 40}
        ...
        {#each lanes as lane, i (lane.plotlineId)}
          ...
          {#each lane.stops as stop, j (stop.sceneId)}
            ...
            <text
              ...
              text-anchor="middle"
              font-size="9"
              fill="var(--text-muted, #aaa)">{stop.sceneName.slice(0, 12)}</text
            >
            {#if j > 0}
          ...
          <text
            ...
            font-weight="600"
            text-anchor="end"
            transform="translate(-8, 0)">{lane.plotlineName.slice(0, 15)}</text
          >
        {/each}
      </svg>
    </div>
    ...
    <div class="pm-list">
            ...
            {#each pl.sceneIds as sceneId}
              ...
              {#if scene}
                <div class="pm-scene-pill">
                  ...
                  <button class="pm-pill-x" on:click={() => removeSceneFromPlotline(pl.id, sceneId)}
                    >×</button
                  >
                </div>
              {/if}
      ...
      {#if $storyPlotlines.length === 0}
        <div class="pm-empty">
          ...
          <p>No plotlines. Create one from the Plot Grid or command palette.</p>
        </div>
      {/if}
    </div>
  {/if}
...
<style>
  ...
  .pm-plotline:hover .pm-pl-delete {
    opacity: 1;
  }
  .pm-pl-scenes {
    display: flex;
