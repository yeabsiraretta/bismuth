<script lang="ts">
  ...
  function handleCompile() {
    ...
    const scenes = $allEntities.filter(
      (e) => e.storyId === $activeStoryId && e.type === 'scene'
    ) as unknown as SceneEntity[];
    const chapters = $allEntities.filter(
      (e) => e.storyId === $activeStoryId && e.type === 'chapter'
    ) as unknown as ChapterEntity[];
    ...
    {#each workflows as wf (wf.id)}
      <div class="cm-wf" class:selected={selectedWorkflowId === wf.id}>
        ...
        {#if !wf.isBuiltIn}
          <button
            class="cm-wf-delete"
            on:click|stopPropagation={() => deleteWorkflow(wf.id)}
            title="Delete"
          >
            <Icon name="x" size={12} />
          </button>
