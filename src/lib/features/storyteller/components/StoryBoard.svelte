<script lang="ts">
  ...
  function moveScene(sceneId: string, newStatus: SceneStatus) {
    ...
    editEntity({
      ...scene,
      status: newStatus,
    } as unknown as import('../types/entity').StorytellerEntity);
  }
        ...
        <div class="sb-col-body">
          {#each col.scenes as scene (scene.id)}
            <div
              ...
              aria-selected="false"
              draggable="true"
              on:dragstart={() => handleDragStart(scene.id)}
            >
              ...
              {/if}
              {#if scene.synopsis}
                <div class="sb-card-synopsis">
                  {scene.synopsis.slice(0, 80)}{scene.synopsis.length > 80 ? '…' : ''}
                </div>
              {/if}
            </div>
          {/each}
