    ...
    <div class="cp-header-actions">
      {#if $storyCampaigns.length > 0}
        <select
          ...
          on:change={(e) => selectCampaign(e.currentTarget.value || null)}
        >
        ...
        <div class="cp-section">
          <div class="cp-add-row">
            ...
            <button
              ...
              on:click={() => {
                if (memberName.trim()) {
                  addPartyMember('', memberName.trim(), memberHp);
                  memberName = '';
                }
              }}>Add</button
            >
          ...
          {#each $activeCampaign.party as member (member.id)}
            <div class="cp-member" class:inactive={!member.isActive}>
              <div class="cp-member-header">
                <button
                  class="cp-toggle"
                  on:click={() => toggleMemberActive(member.id)}
                ...
                <div class="cp-hp">
                  <button
                    class="cp-hp-btn"
                    on:click={() => updateMemberHp(member.id, member.hp - 1)}>−</button
                  >
                  <span class="cp-hp-val" class:low={member.hp <= member.maxHp * 0.25}
              ...
              {#if member.conditions.length > 0}
                <div class="cp-conditions">
                  {#each member.conditions as cond}
                    <button
                      class="cp-condition-tag"
                      on:click={() => removeMemberCondition(member.id, cond)}>{cond} ×</button
                    >
                  {/each}
                </div>
              {/if}
                ...
                <select
                  ...
                  on:change={(e) => {
                    if (e.currentTarget.value) {
                      addMemberCondition(member.id, e.currentTarget.value);
        ...
        <div class="cp-section">
          ...
          {#each $activeCampaign.sharedInventory as item (item.id)}
            <div class="cp-item">
              ...
              {#if item.ownerId}
                <span class="cp-item-owner"
                  >({$activeCampaign.party.find((m) => m.id === item.ownerId)?.name ??
                    'Unknown'})</span
                >
              {/if}
              <select
                class="cp-select-sm"
                value={item.ownerId ?? ''}
                on:change={(e) => assignOwner(item.id, e.currentTarget.value || null)}
              >
      ...
      {:else if tab === 'scenes'}
        <div class="cp-section">
          <div class="cp-empty">
            ...
            <p>
              Scene graph and branching navigation. Link scenes from the Entity panel to use them
              here.
            </p>
          </div>
        </div>
