    ...
    {#if $activeStory}
      ...
      <nav class="st-nav">
        {#each NAV_ITEMS as item}
          <button
            class="st-nav-item"
            class:active={$dashboardView === item.view}
            on:click={() => setDashboardView(item.view)}
          >
            <Icon name={item.icon} size={15} />
            <span>{item.label}</span>
            {#if item.view === 'entities'}
              <span class="st-nav-count"
                >{Object.values($entityCountsByType).reduce((a, b) => a + b, 0)}</span
              >
            {/if}
          </button>
        {/each}
        ...
        <div class="st-story-grid">
          {#each $stories as story (story.id)}
            <div
              ...
              on:keydown={(e) => e.key === 'Enter' && selectStory(story.id)}
              role="button"
              tabindex="0"
            >
              ...
              <button
                class="st-story-delete"
                on:click|stopPropagation={() => removeStory(story.id)}
                title="Delete story"
              >
...
<style>
  ...
  .st-sidebar {
    width: 200px;
    min-width: 200px;
    border-right: 1px solid var(--background-modifier-border, #333);
    display: flex;
  ...
  .st-story-card:hover .st-story-delete {
    opacity: 1;
  }
  .st-story-delete:hover {
