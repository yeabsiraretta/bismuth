...
export function loadPlotlines(): Plotline[] {
  try {
    const r = localStorage.getItem(PLOTLINES_KEY);
...
export function persistPlotlines(plotlines: Plotline[]): void {
  try {
    localStorage.setItem(PLOTLINES_KEY, JSON.stringify(plotlines));
  } catch {
    /* */
  }
}
...
export function loadSetups(): SetupPayoff[] {
  try {
    const r = localStorage.getItem(SETUPS_KEY);
    return r ? JSON.parse(r) : [];
  } catch {
    return [];
  }
...
export function loadPlotgridCells(): Record<string, PlotgridCell> {
  try {
    const r = localStorage.getItem(PLOTGRID_KEY);
    return r ? JSON.parse(r) : {};
  } catch {
    return {};
  }
...
export function persistPlotgridCells(cells: Record<string, PlotgridCell>): void {
  try {
    localStorage.setItem(PLOTGRID_KEY, JSON.stringify(cells));
  } catch {
    /* */
  }
}
