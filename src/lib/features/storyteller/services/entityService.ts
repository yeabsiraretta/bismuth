...
export function persistEntities(entities: StorytellerEntity[]): void {
  try {
    ...
    log.warn('Failed to persist entities', { error: String(e) });
  }
}
...
export function updateEntity(
  ...
  updated: StorytellerEntity
): StorytellerEntity[] {
  return entities.map((e) =>
    e.id === updated.id ? { ...updated, modifiedAt: new Date().toISOString() } : e
  );
}
...
export function getEntitiesByStory(
  entities: StorytellerEntity[],
  storyId: string
): StorytellerEntity[] {
...
export function persistRelationships(rels: EntityRelationship[]): void {
  try {
    ...
    log.warn('Failed to persist relationships', { error: String(e) });
  }
}
...
export function getRelationshipsForEntity(
  ...
  return rels.filter(
    (r) => r.sourceId === entityId || (r.bidirectional && r.targetId === entityId)
  );
}

export function deleteRelationshipsForEntity(
  rels: EntityRelationship[],
  entityId: string
): EntityRelationship[] {
...
export function persistCustomFields(fields: CustomFieldDefinition[]): void {
  try {
    ...
    log.warn('Failed to persist custom fields', { error: String(e) });
  }
}
