...
export function exportProject(
  story: Story, entities: StorytellerEntity[], options: ExportOptions,
): ExportResult {
  ...
  switch (options.format) {
    case 'markdown': return exportMarkdown(story, scenes, chapters, entities, options);
    case 'json': return exportJson(story, entities, options);
    case 'csv': return exportCsv(story, scenes, options);
    case 'html': return exportHtml(story, scenes, chapters, entities, options);
    default: return exportMarkdown(story, scenes, chapters, entities, options);
  }
}
...
function exportMarkdown(
  story: Story, scenes: SceneEntity[], chapters: ChapterEntity[],
  entities: StorytellerEntity[], options: ExportOptions,
): ExportResult {
  let md = `# ${story.name}\n\n`;
...
function buildOutline(scenes: SceneEntity[], chapters: ChapterEntity[], entities: StorytellerEntity[]): string {
  let out = '## Outline\n\n';
  for (const ch of chapters) {
    out += `### ${ch.name}\n\n`;
    const chScenes = scenes.filter(s => s.chapterId === ch.id).sort((a, b) => a.sortOrder - b.sortOrder);
...
function exportJson(story: Story, entities: StorytellerEntity[], options: ExportOptions): ExportResult {
  const data = options.scope === 'outline'
    ? { story: { name: story.name, wordCount: story.wordCount }, entities: entities.map(e => ({ id: e.id, type: e.type, name: e.name, description: e.description.slice(0, 200) })) }
    : { story, entities };
  return { content: JSON.stringify(data, null, 2), filename: `${story.name}.json`, mimeType: 'application/json' };
}
...
function exportCsv(story: Story, scenes: SceneEntity[], options: ExportOptions): ExportResult {
  ...
  const rows = scenes.map(sc =>
    [sc.name, sc.status, sc.pov ?? '', String(sc.wordCount ?? 0), sc.chapterId ?? '', (sc.synopsis ?? '').replace(/"/g, '""')].map(v => `"${v}"`).join(',')
  );
...
function exportHtml(
  ...
  if (options.scope === 'manuscript') {
    ...
    body += `<h2>Outline</h2>\n<ul>\n`;
    for (const sc of scenes) body += `<li><strong>${esc(sc.name)}</strong> [${esc(sc.status)}]</li>\n`;
    body += `</ul>\n`;
  }
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc(story.name)}</title>` +
