...
export function loadStreak(): WritingStreak {
  try { const r = localStorage.getItem(STREAK_KEY); return r ? JSON.parse(r) : { currentDays: 0, longestDays: 0, lastWritingDate: null }; }
  catch { return { currentDays: 0, longestDays: 0, lastWritingDate: null }; }
}
...
export function loadGoals(): WritingGoals {
  try { const r = localStorage.getItem(GOALS_KEY); return r ? { ...DEFAULT_WRITING_GOALS, ...JSON.parse(r) } : { ...DEFAULT_WRITING_GOALS }; }
  catch { return { ...DEFAULT_WRITING_GOALS }; }
}
...
export function loadSprint(): WritingSprint {
  try { const r = localStorage.getItem(SPRINT_KEY); return r ? JSON.parse(r) : { isRunning: false, durationMinutes: 25, startedAt: null, wordsAtStart: 0, wordsWritten: 0 }; }
  catch { return { isRunning: false, durationMinutes: 25, startedAt: null, wordsAtStart: 0, wordsWritten: 0 }; }
}

export function saveSprint(sprint: WritingSprint): void {
  try { localStorage.setItem(SPRINT_KEY, JSON.stringify(sprint)); } catch { /* */ }
}

// ─── Goal progress ──────────────────────────────────────────────────────────

export function getGoalProgress(goals: WritingGoals): { daily: number; weekly: number; monthly: number } {
  const today = new Date().toISOString().slice(0, 10);
  const sessions = loadSessions();
...
export function analyzePacing(scenes: SceneEntity[]): PacingData[] {
  ...
  return scenes.map(sc => ({
    sceneId: sc.id, sceneName: sc.name,
    wordCount: sc.wordCount ?? 0, avgWordCount: Math.round(avg),
...
export function detectPlotHoles(
  scenes: StorytellerEntity[], entities: StorytellerEntity[],
  setups: SetupPayoff[], storyId: string,
): PlotHoleWarning[] {
  ...
  for (const setup of storySetups) {
    if (!setup.resolved && !setup.payoffSceneId) {
      warnings.push({
        id: `unresolved-${setup.id}`, category: 'unresolved-setup', severity: 'warning',
        message: `Unresolved setup: "${setup.description}"`,
        sceneIds: [setup.setupSceneId], entityIds: [],
      });
    }
  }
  ...
  for (const char of characters) {
    const referenced = scenes.some(s => s.tags?.includes(char.name.toLowerCase()) || s.description?.includes(char.name));
    if (!referenced) {
      warnings.push({
        id: `missing-char-${char.id}`, category: 'missing-character', severity: 'info',
        message: `Character "${char.name}" is never referenced in any scene`,
        sceneIds: [], entityIds: [char.id],
      });
    }
  }
  return warnings;
