...
import type { Campaign, PartyMember, InventoryItem, SessionNote, SessionLogEntry, LoreSurface } from '../types/campaign';
...
export function createCampaign(storyId: string, name: string): Campaign {
  return {
    id: crypto.randomUUID(), storyId, name,
    sessionsFolder: 'Sessions',
    activeSessionId: null, activeSceneId: null,
    party: [], sharedInventory: [], logEntries: [],
  };
}
...
export function addPartyMember(campaign: Campaign, entityId: string, name: string, maxHp: number): Campaign {
  const member: PartyMember = {
    id: crypto.randomUUID(), entityId, name,
    hp: maxHp, maxHp, conditions: [], isActive: true,
  };
...
export function updatePartyMemberHp(campaign: Campaign, memberId: string, hp: number): Campaign {
  return {
    ...campaign,
    party: campaign.party.map(m => m.id === memberId ? { ...m, hp: Math.max(0, Math.min(hp, m.maxHp)) } : m),
  };
}
  ...
  return {
    ...
    party: campaign.party.map(m =>
      m.id === memberId && !m.conditions.includes(condition)
        ? { ...m, conditions: [...m.conditions, condition] } : m
    ),
  };
}
...
export function assignItemOwner(campaign: Campaign, itemId: string, ownerId: string | null): Campaign {
  return {
    ...campaign,
...
export function addLogEntry(campaign: Campaign, sessionId: string, type: SessionLogEntry['type'], text: string): Campaign {
...
export function createSessionNote(campaignId: string, sessionNumber: number, title: string, notePath: string): SessionNote {
  return { id: crypto.randomUUID(), campaignId, sessionNumber, title, notePath, date: new Date().toISOString() };
}

// ─── Lore surfacing ─────────────────────────────────────────────────────────

export function surfaceLore(
  entities: StorytellerEntity[], campaign: Campaign,
): LoreSurface[] {
  ...
  for (const entity of entities) {
    if (partyEntityIds.has(entity.id)) {
      results.push({ entityId: entity.id, entityName: entity.name, entityType: entity.type, relevance: 'character', snippet: entity.description.slice(0, 120) });
    }
    if (inventoryNames.has(entity.name.toLowerCase())) {
      results.push({ entityId: entity.id, entityName: entity.name, entityType: entity.type, relevance: 'inventory', snippet: entity.description.slice(0, 120) });
    }
  }
  return results;
}
