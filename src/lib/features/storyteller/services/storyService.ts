...
export function loadConfig(): StorytellerConfig {
  try {
    ...
    return raw
      ? { ...DEFAULT_STORYTELLER_CONFIG, ...JSON.parse(raw) }
      : { ...DEFAULT_STORYTELLER_CONFIG };
  } catch {
    return { ...DEFAULT_STORYTELLER_CONFIG };
  }
}
...
export function saveConfig(config: StorytellerConfig): void {
  try {
    ...
    log.warn('Failed to save storyteller config', { error: String(e) });
  }
}
...
export function persistStories(stories: Story[]): void {
  try {
    ...
    log.warn('Failed to persist stories', { error: String(e) });
  }
}
...
export function createStory(name: string, config: StorytellerConfig): Story {
  ...
  const rootPath = config.oneStoryMode ? config.rootFolder : `${config.rootFolder}/Stories/${name}`;
  const story: Story = {
    id,
    name,
    description: '',
...
export function getEntityFolderPath(
  story: Story,
...
export function updateStory(stories: Story[], updated: Story): Story[] {
  return stories.map((s) =>
    s.id === updated.id ? { ...updated, modifiedAt: new Date().toISOString() } : s
  );
}
