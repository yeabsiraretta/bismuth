...
export function persistForks(forks: TimelineFork[]): void {
  try {
    localStorage.setItem(FORKS_KEY, JSON.stringify(forks));
  } catch {
    /* silent */
  }
}
...
export function filterTimelineEvents(
  ...
  if (filter.tracks.length > 0)
    result = result.filter((e) => e.track && filter.tracks.includes(e.track));
  ...
  for (let i = 0; i < sorted.length; i++) {
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      if (a.track && b.track && a.track === b.track) {
        conflicts.push({
          eventIds: [a.id, b.id],
          reason: `Overlapping events on track "${a.track}"`,
          severity: 'warning',
        });
      }
    }
