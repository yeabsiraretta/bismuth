...
export const trackerSettings = writable<TrackerSettings>(
  loadJson(SETTINGS_KEY, DEFAULT_TRACKER_SETTINGS)
);
...
export const captureProgress = derived([captureState, propertyDefs], ([$s, $d]) =>
  $d.length > 0 ? Math.round(($s.currentIndex / $d.length) * 100) : 0
);
