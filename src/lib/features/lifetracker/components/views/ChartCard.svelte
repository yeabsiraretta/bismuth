<script lang="ts">
  import { aggregateTimeSeries, movingAverage, computeTrend, detectScale, formatDateLabel } from '../../services/aggregation';
  import { trackerSettings } from '../../stores/trackerStore';
  import { COLOR_SCHEMES } from '../../types';
  import type { DataPoint, VisualizationType, ColorScheme, AggregationMethod, ScaleConfig, ReferenceLine } from '../../types';
  ...
  function x(i: number): number { return PAD.left + (series.length > 1 ? (i / (series.length - 1)) * innerW : innerW / 2); }
  function y(val: number): number { return PAD.top + innerH - ((val - effectiveScale.min) / range) * innerH; }

  $: primaryColor = COLOR_SCHEMES[colorScheme].levels[2];
  $: areaColor = `${primaryColor}33`;

  $: linePath = series.map((p, i) => `${i === 0 ? 'M' : 'L'}${x(i).toFixed(1)},${y(p.value).toFixed(1)}`).join(' ');
  $: areaPath = series.length > 0
  ...
  $: xLabels = series.length <= 12
    ? series.map((p, i) => ({ label: formatDateLabel(p.date, $trackerSettings.granularity), x: x(i) }))
    : series.filter((_, i) => i % Math.ceil(series.length / 8) === 0).map((p, i) => ({
...
<div class="chart-card">
  <div class="chart-header">
    ...
    {/if}
    <span class="trend-arrow" class:up={trend.direction === 'up'} class:down={trend.direction === 'down'} title="{trend.percentChange}% {trend.direction}">
      {trend.arrow}
    </span>
  </div>
  ...
  <svg viewBox="0 0 {CHART_W} {CHART_H}" class="chart-svg" preserveAspectRatio="xMidYMid meet">
    {#if showGrid}
      {#each gridLines as g}
        ...
        <text x={PAD.left - 4} y={g.y + 3} class="axis-label" text-anchor="end">{g.val.toFixed(0)}</text>
      {/each}
    {/if}

    {#each referenceLines as ref}
      <line x1={PAD.left} y1={y(ref.value)} x2={CHART_W - PAD.right} y2={y(ref.value)} stroke={ref.color} stroke-dasharray="4 2" stroke-width="1" />
      <text x={CHART_W - PAD.right + 2} y={y(ref.value) + 3} fill={ref.color} font-size="8">{ref.label}</text>
    {/each}
    ...
    {:else if visualization === 'line'}
      ...
      {#each series as p, i}
        <circle cx={x(i)} cy={y(p.value)} r="3" fill={primaryColor}>
          <title>{formatDateLabel(p.date, $trackerSettings.granularity)}: {p.value.toFixed(1)}</title>
        </circle>
      {/each}
    ...
    {#if maPath}
      <path d={maPath} fill="none" stroke={primaryColor} stroke-width="1.5" stroke-dasharray="5 3" opacity="0.7" />
    {/if}
...
<style>
  .chart-card { padding: var(--spacing-s); }
  .chart-header { display: flex; align-items: center; gap: var(--spacing-xs); margin-bottom: var(--spacing-xs); }
  .card-title { font-size: var(--font-ui-small); font-weight: 600; color: var(--text-normal); margin: 0; flex: 1; }
  .trend-arrow { font-size: 14px; font-weight: 700; color: var(--text-muted); }
  .trend-arrow.up { color: #10b981; }
  .trend-arrow.down { color: #ef4444; }
  .chart-svg { width: 100%; height: auto; display: block; }
  .grid-line { stroke: var(--background-modifier-border, rgba(128,128,128,0.15)); stroke-width: 0.5; }
  .axis-label { font-size: 8px; fill: var(--text-muted); }
  .chart-footer { display: flex; gap: var(--spacing-m); margin-top: var(--spacing-xs); }
  .stat { font-size: 10px; color: var(--text-muted); }
  .stat.ma { font-style: italic; }
</style>
