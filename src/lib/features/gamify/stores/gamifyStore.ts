...
export function purchaseReward(rewardId: string): boolean {
  ...
  gamifyRewards.update((r) =>
    r.map((rw) => (rw.id === rewardId ? { ...rw, purchased: true } : rw))
  );
  gamifyHistory.update((h) => [
    ...h,
    {
...
export function resetReward(rewardId: string): void {
  gamifyRewards.update((r) =>
    r.map((rw) => (rw.id === rewardId ? { ...rw, purchased: false } : rw))
  );
}
