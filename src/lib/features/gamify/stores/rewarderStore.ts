...
function persistConfig(config: RewarderConfig): void {
  ...
  catch (e) { log.warn('rewarder: config persist failed', { error: String(e) }); }
}
...
function persistHistory(history: AwardedReward[]): void {
  ...
  catch (e) { log.warn('rewarder: history persist failed', { error: String(e) }); }
}

export const awardedRewards = writable<AwardedReward[]>(loadHistory());
awardedRewards.subscribe(persistHistory);

export const recentAwards = derived(awardedRewards, $h =>
...
export function skipPendingReward(): void {
  ...
  if (reward) {
    awardedRewards.update(h => h.map(a =>
      a.id === reward.id ? { ...a, skipped: true } : a
    ));
...
export async function rollRewardForTask(
  ...
  if (winner.inventory !== null && writeFile) {
    ...
    if (updated) {
      try { await writeFile(config.rewardsFile, updated); }
      catch (e) { log.warn('rewarder: failed to update inventory', { error: String(e) }); }
    }
  }
  ...
  if (config.saveToDaily && appendToDaily) {
    ...
    try { await appendToDaily(config.dailySectionHeading, line); }
    catch (e) { log.warn('rewarder: failed to write daily note reward', { error: String(e) }); }
  }

  if (config.saveTaskToDaily && appendToDaily) {
    const time = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const line = `- [x] ${taskText} _(completed ${time})_`;
    try { await appendToDaily(config.taskSectionHeading, line); }
    catch (e) { log.warn('rewarder: failed to write daily note task', { error: String(e) }); }
  }
