...
<style>
  ...
  .balance {
    ...
    background: var(--panel-bg-alt, #181825); border-radius: 6px;
  }
  .balance-amount { font-size: 14px; font-weight: 700; color: var(--warning-color, #f59e0b); }
  .reward-section { display: flex; flex-direction: column; gap: 4px; }
  ...
  .reward-item.purchased { opacity: 0.7; }
  .reward-icon { font-size: 18px; }
  .reward-info { flex: 1; display: flex; flex-direction: column; gap: 2px; }
  .reward-name { font-size: 12px; font-weight: 500; }
  .reward-desc { font-size: 10px; color: var(--text-muted); }
  .buy-btn {
    padding: 4px 10px; border: none; border-radius: 4px;
    background: var(--warning-color, #f59e0b); color: white;
  ...
  .empty { text-align: center; padding: 16px; color: var(--text-muted); }
