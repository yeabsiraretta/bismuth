...
calendarViewMode.subscribe((mode) => {
  try {
    ...
    log.warn('Failed to persist calendar view mode', { error: String(e) });
  }
});
...
function loadPlannerSettings(): PlannerSettings {
  try {
    ...
    return stored
      ? { ...DEFAULT_PLANNER_SETTINGS, ...JSON.parse(stored) }
...
plannerSettings.subscribe((s) => {
  try {
    localStorage.setItem(PLANNER_SETTINGS_KEY, JSON.stringify(s));
  } catch (e) {
    log.warn('Failed to persist planner settings', { error: String(e) });
  }
});
...
export const expandedEvents = derived([allCalendarItems, calendarFocusDate], ([$items, $focus]) => {
...
export function updateCalendarEvent(id: string, updates: Partial<CalendarEvent>): void {
  calendarEvents.update((events) => events.map((e) => (e.id === id ? { ...e, ...updates } : e)));
}
...
calendarEvents.subscribe((events) => {
  try {
    ...
    log.warn('Failed to persist calendar events to localStorage', { error: String(e) });
  }
});
