...
import { calendarEvents, addCalendarEvent, deleteCalendarEvent, updateCalendarEvent } from './calendarStore';
...
export function deselectEvent(id: string): void {
  selectedEventIds.update(s => { const n = new Set(s); n.delete(id); return n; });
}
...
export function batchDelete(): void {
  ...
  pushUndo({ id: generatePrefixedId('undo'), type: 'batch', label: `Delete ${ids.length} events`, timestamp: Date.now(), before, after: [] });
...
export function batchComplete(): void {
  ...
  pushUndo({ id: generatePrefixedId('undo'), type: 'batch', label: `Complete ${ids.length} events`, timestamp: Date.now(), before, after });
  clearSelection();
}
...
export function batchDuplicate(): void {
  ...
  pushUndo({ id: generatePrefixedId('undo'), type: 'batch', label: `Duplicate ${ids.length} events`, timestamp: Date.now(), before: [], after: copies });
  clearSelection();
...
export function batchShift(days: number): void {
  ...
  const label = days > 0 ? `Shift ${ids.length} events forward ${days}d` : `Shift ${ids.length} events back ${Math.abs(days)}d`;
  pushUndo({ id: generatePrefixedId('undo'), type: 'batch', label, timestamp: Date.now(), before, after });
  clearSelection();
...
export function batchMove(targetDate: string): void {
  ...
  pushUndo({ id: generatePrefixedId('undo'), type: 'batch', label: `Move ${ids.length} events to ${targetDate}`, timestamp: Date.now(), before, after });
  clearSelection();
}
...
export function trackedAdd(event: CalendarEvent): void {
  ...
  pushUndo({ id: generatePrefixedId('undo'), type: 'add', label: `Add "${event.title}"`, timestamp: Date.now(), before: [], after: [event] });
}

export function trackedDelete(id: string): void {
  const before = snapshotEvents([id]);
  deleteCalendarEvent(id);
  pushUndo({ id: generatePrefixedId('undo'), type: 'delete', label: `Delete "${before[0]?.title ?? id}"`, timestamp: Date.now(), before, after: [] });
}
...
export function trackedUpdate(id: string, updates: Partial<CalendarEvent>): void {
  ...
  pushUndo({ id: generatePrefixedId('undo'), type: 'update', label: `Edit "${before[0]?.title ?? id}"`, timestamp: Date.now(), before, after });
}
