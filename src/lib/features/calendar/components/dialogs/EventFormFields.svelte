  ...
  <div class="type-selector" id="event-type" role="group" aria-label="Event type">
    {#each [['event','Event'],['task','Task'],['time-block','Time Block']] as [val,label]}
      <button
        ...
        on:click={() => onChange?.({ itemType: val as CalendarItemType })}
      >{label}</button>
    {/each}
  </div>
</div>
...
<div class="form-row">
  ...
  <div class="category-row">
    {#if selectedCategoryColor}
      <span class="category-swatch" style="background: {selectedCategoryColor}" aria-hidden="true"></span>
    {/if}
    <select
      id="event-category"
...
<div class="form-row">
  ...
  <input
    id="event-date" type="date" class="input-field"
    value={eventDate}
...
{#if !allDay}
  <div class="form-row time-row">
    ...
    <select id="event-start-hour" class="input-field time-select" value={startHour}
    ...
    <select class="input-field time-select" value={startMin}
  ...
  <div class="form-row time-row">
    ...
    <select id="event-duration-hours" class="input-field time-select" value={durationHours}
      on:change={e => onChange?.({ durationHours: Number((e.target as HTMLSelectElement).value) })}>
      {#each Array.from({length: 13}, (_, i) => i) as h}
        <option value={h}>{h}h</option>
      {/each}
    </select>
    <select class="input-field time-select" value={durationMins}
...
<style>
  ...
  .input-title:focus { border-color: var(--interactive-accent); }
  .form-row { display: flex; align-items: center; gap: 10px; }
  .form-label { font-size: 0.75rem; color: var(--text-muted); min-width: 60px; display: flex; align-items: center; gap: 6px; }
  .type-selector { display: flex; border: 1px solid var(--border-color); border-radius: var(--radius-s); overflow: hidden; }
  .type-btn { padding: 5px 12px; font-size: 0.72rem; border: none; background: var(--background-primary); color: var(--text-muted); cursor: pointer; min-height: 40px; }
  .type-btn:not(:last-child) { border-right: 1px solid var(--border-color); }
  .type-btn.active { background: var(--interactive-accent); color: var(--text-on-accent); }
  .category-row { display: flex; align-items: center; gap: 6px; flex: 1; }
  .category-swatch { width: 14px; height: 14px; border-radius: 3px; flex-shrink: 0; border: 1px solid rgba(0,0,0,0.1); }
  .category-select { flex: 1; }
  .input-field { padding: 6px 10px; border: 1px solid var(--border-color); border-radius: var(--radius-s); background: var(--background-secondary); color: var(--text-normal); font-size: 0.8rem; min-height: 36px; }
  .time-row { gap: 8px; }
