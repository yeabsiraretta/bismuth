...
import type { TypeDefinition, EntityRelationships } from '@/types/data/entity';
...
export async function updateFrontmatterField(path: string, key: string, value: unknown): Promise<void> {
	return invoke<void>('update_frontmatter_field', { path, key, value });
}

/** Load custom entity types from vault config. */
export async function getCustomEntityTypes(): Promise<TypeDefinition[]> {
	return invoke<TypeDefinition[]>('get_custom_entity_types');
}
