...
{#if source}
  <div class="media-player" class:pinned={state.pinned}>
    ...
    <div class="player-viewport">
      ...
      {/if}
      {#if state.buffering}
        <div class="buffering-overlay"><Icon name="loader" size={24} /></div>
        <!--end-->
      {/if}
    </div>
    ...
    <div class="player-controls">
      ...
      <div class="button-row">
        <button
          class="ctrl-btn"
          on:click={() => seekTo(Math.max(0, state.currentTime - 5))}
          title="Back 5s"
        ...
        <button
          class="ctrl-btn"
          on:click={() => seekTo(Math.min(state.duration, state.currentTime + 5))}
          title="Forward 5s"
        ...
        <button
          class="ctrl-btn"
          on:click={handleInsertTimestamp}
          title="Insert timestamp"
          aria-label="Insert timestamp at cursor"
        >
...
<style>
  ...
  .media-player.pinned {
    border-color: var(--interactive-accent, #89b4fa);
    box-shadow: 0 0 0 1px var(--interactive-accent, #89b4fa);
  }
  .player-header {
    display: flex;
    align-items: center;
  ...
  .scrubber {
    flex: 1;
    accent-color: var(--interactive-accent, #89b4fa);
    height: 4px;
    cursor: pointer;
  }
  .button-row {
