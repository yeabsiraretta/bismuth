<script lang="ts">
  ...
  async function openFile(): Promise<void> {
    try {
      ...
      if (typeof selected === 'string') {
        ...
        log.info('ConvertPanel: opened', {
          path: selected,
          w: srcDimensions.width,
          h: srcDimensions.height,
        });
      }
    } catch (err) {
  ...
  async function handleConvert(): Promise<void> {
    ...
    try {
      ...
      const destPath = buildOutputPath(
        dir + '/media-edits',
        renameConfig,
        config.outputFormat,
        ctx
          ...
          <div class="chip-row">
            {#each formats as fmt}
              <FilterChip
                ...
                on:click={() => {
                  config = { ...config, outputFormat: fmt };
                }}
              />
            {/each}
          </div>
              ...
              <div class="chip-row wrap">
                {#each resizeModes as mode}
                  <FilterChip
                    ...
                    on:click={() => {
                      config = { ...config, resizeMode: mode.id };
                    }}
                  />
                {/each}
              </div>
          ...
          {#if config.flipEnabled}
            <div class="chip-row">
              <FilterChip
                ...
                on:click={() => {
                  config = { ...config, flipDirection: 'horizontal' };
                }}
              />
              <FilterChip
                label="Vertical"
                ...
                on:click={() => {
                  config = { ...config, flipDirection: 'vertical' };
                }}
              />
            </div>
          {/if}
          ...
          <div class="rename-section">
            ...
            {#if renameConfig.enabled}
              <label class="inline-label" for="cv-rename-pattern">
                Pattern
                <input
                  id="cv-rename-pattern"
                  type="text"
                  bind:value={renameConfig.pattern}
                  class="text-input"
                  placeholder="{'{noteName}'}-{'{fileName}'}"
                />
              </label>
              <div class="var-hints">
                {#each RENAME_VARIABLES as v}
                  <button
                    class="var-chip"
                    on:click={() => {
                      renameConfig = { ...renameConfig, pattern: renameConfig.pattern + v };
                    }}>{v}</button
                  >
                {/each}
              </div>
            ...
            <label class="inline-label" for="cv-output-folder">
              ...
              <input
                id="cv-output-folder"
                type="text"
                bind:value={renameConfig.outputFolder}
                class="text-input"
