<script lang="ts">
  ...
  export let onTrimChange: ((detail: { start: number; end: number }) => void) | undefined =
    undefined;
  ...
  function handleTrackKeydown(e: KeyboardEvent): void {
    ...
    if (e.key === 'ArrowLeft') {
      e.preventDefault();
      onScrub?.({ time: clamp(currentTime - step, 0, duration) });
    }
    if (e.key === 'ArrowRight') {
      e.preventDefault();
      onScrub?.({ time: clamp(currentTime + step, 0, duration) });
...
<div class="video-timeline" class:disabled>
  ...
  >
    ...
    <div class="trim-region" style="left: {trimStartPct}%; width: {trimWidthPct}%"></div>

    <!-- Playhead -->
    <div class="playhead" style="left: {playheadPct}%" aria-hidden="true"></div>
