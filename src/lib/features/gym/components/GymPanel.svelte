<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      const vault = get(currentVault as import('svelte/store').Readable<{ root_path: string; id?: string } | null>);
...
<style>
  .gym-panel { display: flex; flex-direction: column; height: 100%; }
  .tab-bar { display: flex; border-bottom: 1px solid var(--background-modifier-border); }
  .tab-btn {
    ...
    cursor: pointer; font-size: var(--font-ui-small); transition: color 0.15s;
  }
  ...
  .loading-vault { display: flex; align-items: center; justify-content: center; height: 100%; color: var(--text-muted); font-size: var(--font-ui-small); }
  .history-tab { padding: var(--spacing-m); }
  .history-title { font-size: var(--font-ui-small); font-weight: 600; color: var(--text-normal); margin: 0 0 var(--spacing-m); }
  .empty-state { color: var(--text-muted); font-size: var(--font-ui-small); }
  .session-list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: var(--spacing-xs); }
  .session-item {
    display: flex; justify-content: space-between; align-items: center;
    padding: var(--spacing-s) var(--spacing-m); background: var(--background-secondary);
    border-radius: var(--radius-s);
  }
  .session-date { font-size: var(--font-ui-small); color: var(--text-normal); }
  .session-duration { font-size: var(--font-ui-small); color: var(--text-muted); }
</style>
