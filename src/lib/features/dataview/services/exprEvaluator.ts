...
export function evaluateExpr(expr: DvExpr, page: DvPage): DvValue {
  switch (expr.type) {
    case 'literal':
      return expr.value;
    case 'field':
      return resolveField(expr.path, page);
    case 'binary':
      return evaluateBinary(expr.op, evaluateExpr(expr.left, page), evaluateExpr(expr.right, page));
    case 'unary':
      return evaluateUnary(expr.op, evaluateExpr(expr.operand, page));
    case 'function':
      return evaluateFunction(
        expr.name,
        expr.args.map((a) => evaluateExpr(a, page))
      );
  }
}
...
function evaluateFunction(name: string, args: DvValue[]): DvValue {
  switch (name.toLowerCase()) {
    ...
    case 'dateformat':
      return args[0] instanceof Date ? args[0].toLocaleDateString() : String(args[0] ?? '');
    case 'round':
    ...
    case 'avg': {
      const nums =
        args.length === 1 && Array.isArray(args[0]) ? args[0].map(toNumber) : args.map(toNumber);
    ...
    case 'reverse':
      return Array.isArray(args[0])
        ...
        : String(args[0] ?? '')
            ...
            .join('');
    case 'sort':
      return Array.isArray(args[0]) ? [...args[0]].sort((a, b) => compareValues(a, b)) : args[0];
    case 'flat':
      return Array.isArray(args[0]) ? args[0].flat() : args[0];
    case 'slice': {
    ...
    case 'padleft':
      return String(args[0] ?? '').padStart(toNumber(args[1] ?? 0), String(args[2] ?? ' '));
    case 'padright':
      return String(args[0] ?? '').padEnd(toNumber(args[1] ?? 0), String(args[2] ?? ' '));
    case 'trim':
    ...
    case 'nonnull':
      return Array.isArray(args[0])
        ? args[0].filter((v) => v !== null)
        : args.filter((v) => v !== null);
    case 'all':
      return Array.isArray(args[0]) ? args[0].every(isTruthy) : args.every(isTruthy);
    case 'any':
      return Array.isArray(args[0]) ? args[0].some(isTruthy) : args.some(isTruthy);
    case 'none':
      return Array.isArray(args[0]) ? !args[0].some(isTruthy) : !args.some(isTruthy);
    case 'typeof':
