...
<div class="rss-panel" role="tabpanel" aria-label="RSS Feeds">
  <PanelHeader icon="rss" title="RSS" count={$totalUnread || undefined}>
    <svelte:fragment slot="actions">
      <ActionButton icon="plus" title="Add feed" on:click={() => { showAddForm = !showAddForm; }} />
      ...
      {#each Object.entries($feedsByFolder) as [folder, feeds]}
        ...
        {#each feeds as feed}
          <div class="feed-row">
            ...
            <button class="remove-btn" title="Unsubscribe" on:click={() => unsubscribeFeed(feed.id)}>
              <Icon name="x" size={10} />
            </button>
          </div>
        {/each}
...
<style>
  .rss-panel { display: flex; flex-direction: column; height: 100%; }
  .panel-body { padding: 8px; overflow-y: auto; flex: 1; }
  .add-form { display: flex; gap: 4px; padding: 4px 0 8px; }
  .add-input {
  ...
  .add-btn {
    ...
    background: var(--accent-color, #6366f1); color: white; font-size: 12px; cursor: pointer;
  }
  ...
  .feed-row .feed-item { flex: 1; }
  .remove-btn {
    background: none; border: none; padding: 4px; border-radius: 4px;
    cursor: pointer; color: var(--text-muted); opacity: 0;
  }
  .feed-row:hover .remove-btn { opacity: 1; }
  .remove-btn:hover { color: var(--error-color, #f87171); background: var(--hover-bg); }
</style>
