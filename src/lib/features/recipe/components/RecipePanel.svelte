...
<div class="recipe-panel">
    ...
    <div class="quick-controls">
      ...
      <div class="step-tracker">
        ...
        {#if state.highlightedStep !== null}
          ...
          <button
            class="nav-btn"
            on:click={() => highlightStep(null)}
            aria-label="Clear step highlight"
          >
            <Icon name="x" size={12} />
          </button>
        {:else}
  ...
  {:else if note && !isRecipe}
    <div class="empty-state">
      ...
      <p class="hint">
        Add <strong>## Ingredients</strong> and <strong>## Instructions</strong> headings to enable recipe
        view.
      </p>
      <button
        class="force-btn"
        on:click={() => {
          if (note) loadRecipe(note.content, note.title);
        }}
        aria-label="Force load as recipe"
      >
        Load anyway
      </button>
    </div>
