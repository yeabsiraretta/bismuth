...
<div class="sg-settings">
  ...
  <div class="sg-section">
    ...
    <label>
      ...
      <div class="sg-toggle">
        <button class:active={settings.connectionMode === 'note'} on:click={() => setMode('note')}
          >Note</button
        >
        <button class:active={settings.connectionMode === 'block'} on:click={() => setMode('block')}
          >Block</button
        >
      </div>
    </label>
...
<style>
  ...
  .sg-reset:hover {
    background: var(--graph-surface-hover, var(--interactive-hover));
