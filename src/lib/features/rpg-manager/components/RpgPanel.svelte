...
<div class="rpg-panel">
  <PanelHeader icon="shield">
    ...
    <svelte:fragment slot="actions">
      <button
        class="rpg-btn-sm"
        on:click={() => (showNewCampaign = !showNewCampaign)}
        title="New Campaign"
      >
  ...
  <div class="rpg-body">
    ...
    {#if showNewCampaign}
      <div class="rpg-form">
        <input
          ...
          on:keydown={(e) => e.key === 'Enter' && handleCreateCampaign()}
        />
    ...
    {#if $campaigns.length > 0}
      <div class="rpg-campaign-select">
        <select
          ...
          on:change={() => $activeCampaignId && selectCampaign($activeCampaignId)}
        >
    ...
    {#if $activeCampaign}
      <div class="rpg-actions-bar">
        <button
          class="rpg-btn-sm"
          on:click={() => (showNewElement = !showNewElement)}
          title="New Element"
        >
          <Icon name="plus" size={12} /> New
        </button>
        <button
          class="rpg-btn-sm danger"
          on:click={() => $activeCampaignId && removeCampaign($activeCampaignId)}
      ...
      {#if showNewElement}
        <div class="rpg-form">
          ...
          <input
            ...
            placeholder="{ELEMENT_META[newElementType].label} name..."
            on:keydown={(e) => e.key === 'Enter' && handleCreateElement()}
          />
          <div class="rpg-form-actions">
...
<style>
  ...
  .rpg-btn-sm.danger:hover {
    color: var(--text-error);
    border-color: var(--text-error);
  }
  .rpg-campaign-select select {
    width: 100%;
    font-size: 0.8rem;
    padding: 5px 8px;
    border: 1px solid var(--border-color);
  ...
  .rpg-element-item:hover {
    background: var(--background-modifier-hover);
  }
  .rpg-element-item.active {
