<script lang="ts">
  ...
  async function selectVersion(doc: DesignDocumentMeta, version: number) {
    ...
    if (version < doc.version) {
      ...
      if (oldPayload && newPayload)
        diff = computeDocumentDiff(oldPayload, newPayload, version, doc.version);
    } else {
      diff = null;
    }
  }

  $: counts = TYPE_ORDER.reduce(
    (acc, t) => {
  ...
  $: groupedDocTypes = TYPE_ORDER.map((type) => ({
    type,
    meta: TYPE_META[type],
  ...
  <div class="ddp-body">
    ...
    {:else if documents.length === 0}
      <div class="ddp-state ddp-empty">
        ...
        <p class="ddp-hint">
          Save a canvas to generate documents, or sync using the controls above.
        </p>
      </div>
    {:else}
            ...
            {#each group.docs as doc (doc.document_id)}
              <li class="ddp-item" class:expanded={expandedDocId === doc.document_id}>
                <button class="ddp-row" on:click={() => toggleExpand(doc)}>
                  ...
                  <Icon
                    name={expandedDocId === doc.document_id ? 'chevron-down' : 'chevron-right'}
                    size={12}
                  />
                </button>
                {#if expandedDocId === doc.document_id}
                  <div class="ddp-detail">
                    <div class="detail-meta">
                      <span class="meta-item"
                        ><Icon name="clock" size={11} />{new Date(
                          doc.modified_at * 1000
                        ).toLocaleDateString()}</span
                      >
                      <span class="meta-item"
                        ><Icon name="git-commit" size={11} />v{doc.version}</span
                      >
                    </div>
...
<style>
  ...
  .ddp-detail {
    padding: 8px var(--spacing-m) 10px;
