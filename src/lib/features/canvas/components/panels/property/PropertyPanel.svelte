<script lang="ts">
  ...
  import { currentCanvas, selectedElements, updateElement } from '@/features/canvas/stores';
  import {
    tokenCollections,
    activeMode,
    resolveToken,
  } from '@/features/canvas/stores/design/tokenStore';
  ...
  $: {
    if ($selectedElements.length === 1 && $currentCanvas) {
      selectedElement = $currentCanvas.elements.find((e) => e.id === $selectedElements[0]) || null;
    } else {
  ...
  $: componentDef =
    isInstance && selectedElement?.properties.definitionId
