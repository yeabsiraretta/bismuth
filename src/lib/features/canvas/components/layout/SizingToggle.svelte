...
<div class="sizing-toggle">
  <button class="toggle-btn" on:click={cycle} title={sizingOptions.find(o => o.value === sizing)?.label}>
...
<style>
  .sizing-toggle { display: flex; align-items: center; gap: var(--spacing-xs); }
  .toggle-btn { display: flex; align-items: center; gap: 4px; padding: 2px 6px; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--bg-secondary); cursor: pointer; font-size: var(--font-size-sm); }
  .toggle-btn:hover { background: var(--bg-hover); }
  .icon { font-size: 0.9em; }
  .label { text-transform: capitalize; }
  .constraints { display: flex; gap: 4px; }
  .bound { font-size: var(--font-size-xs); color: var(--text-muted); padding: 0 3px; background: var(--bg-tertiary); border-radius: 2px; }
</style>
