...
export function generateDocuments(canvasDoc: CanvasDocument): DesignDocumentAny[] {
  ...
  for (const def of canvasDoc.components) {
    ...
    docs.push(
      envelope('component', `comp_${def.id}`, def.name, canvasDoc, componentsPageId, payload)
    );
  }
  ...
  if (layoutPayload) {
    docs.push(
      envelope(
        'layout',
        `layout_${layoutPayload.layout_name}`,
        layoutPayload.layout_name,
        canvasDoc,
        layoutsPageId,
        layoutPayload
      )
  ...
  if (pageElements.length > 0) {
    const pagePayload = extractPage(
      canvasDoc.elements,
      canvasDoc.name,
      layoutPayload?.layout_name || 'default'
    );
    if (pagePayload) {
      docs.push(
        envelope(
