...
<div class="tk-panel">
  ...
  <div class="tk-tabs">
    {#each $timekeeps as tk (tk.id)}
      <button
        class="tk-tab"
        class:active={selectedTkId === tk.id}
    ...
    <div class="tk-new-row">
      <input
        type="text"
        bind:value={newTimekeepTitle}
        placeholder="New timekeep..."
        class="tk-new-input"
        on:keydown={(e) => e.key === 'Enter' && handleAddTimekeep()}
      />
      <button class="tk-add-btn" on:click={handleAddTimekeep} title="Create timekeep"
    ...
    <div class="tk-content">
      <div class="tk-content-header">
        <input
          ...
          class="tk-rename"
          value={tk.title}
          on:blur={(e) => renameTimekeep(tk.id, e.currentTarget.value)}
          on:keydown={(e) => {
        ...
        <div class="tk-export-btns">
          <button
            class="tk-sm-btn"
            on:click={() => handleExport(tk.id, 'markdown')}
            title="Copy as Markdown"><Icon name="file-text" size={11} /></button
          >
          <button class="tk-sm-btn" on:click={() => handleExport(tk.id, 'csv')} title="Copy as CSV"
            ><Icon name="grid" size={11} /></button
          >
          <button
            class="tk-sm-btn"
            on:click={() => handleExport(tk.id, 'json')}
            title="Copy as JSON"><Icon name="code" size={11} /></button
          >
          <button
            class="tk-sm-btn tk-danger"
            on:click={() => {
              removeTimekeep(tk.id);
              selectedTkId = null;
            }}
            title="Delete timekeep"><Icon name="trash-2" size={11} /></button
          >
        </div>
      </div>
      ...
      <div class="tk-entries">
        {#each tk.data.entries as entry, idx (idx)}
          <div
            class="tk-entry"
            class:running={isRunning(entry)}
            class:completed={isCompleted(entry)}
          >
            <div class="tk-entry-row">
              {#if entry.subEntries && entry.subEntries.length > 0}
                <button
                  class="tk-collapse"
                  on:click={() => toggleEntryCollapsed(tk.id, idx)}
              ...
              {:else}
                <button
                  class="tk-entry-name-btn"
                  on:dblclick={() => handleStartEdit(tk.id, idx, entry.name)}
                  title="Double-click to edit">{entry.name}</button
                >
              {/if}
              ...
              <div class="tk-entry-actions">
                {#if isNotStarted(entry)}
                  <button class="tk-play" on:click={() => startEntryTimer(tk.id, idx)} title="Start"
                    ><Icon name="play" size={11} /></button
                  >
                {:else if isRunning(entry)}
                  <button class="tk-stop" on:click={() => stopEntryTimer(tk.id, idx)} title="Stop"
                    ><Icon name="square" size={11} /></button
                  >
                {:else}
                  <button
                    class="tk-play"
                    on:click={() => startEntryTimer(tk.id, idx)}
                    title="Restart"><Icon name="refresh-cw" size={11} /></button
                  >
                {/if}
                <button class="tk-del" on:click={() => removeEntry(tk.id, idx)} title="Delete"
                ...
                <div class="tk-sub-entry" class:running={isRunning(sub)}>
                  ...
                  <div class="tk-entry-actions">
                    {#if isNotStarted(sub)}
                      <button
                        class="tk-play"
                        on:click={() => startSubEntryTimer(tk.id, idx, si)}
                        title="Start"><Icon name="play" size={10} /></button
                      >
                    {:else if isRunning(sub)}
                      <button
                        class="tk-stop"
                        on:click={() => stopSubEntryTimer(tk.id, idx, si)}
                        title="Stop"><Icon name="square" size={10} /></button
                      >
                    {/if}
                    <button
                      class="tk-del"
                      on:click={() => removeSubEntryAction(tk.id, idx, si)}
                      title="Delete"><Icon name="x" size={10} /></button
                    >
                  </div>
                </div>
              {/each}
            ...
            <div class="tk-sub-add">
              <input
                ...
                bind:value={newSubEntryNames[`${tk.id}-${idx}`]}
                on:keydown={(e) => e.key === 'Enter' && handleAddSubEntry(tk.id, idx)}
                class="tk-sub-input"
              />
              <button
                class="tk-sub-add-btn"
                on:click={() => handleAddSubEntry(tk.id, idx)}
                title="Add sub-entry"><Icon name="plus" size={10} /></button
              >
            </div>
          </div>
        {/each}
      ...
      <div class="tk-add-entry">
        <input
          type="text"
          bind:value={newEntryNames[tk.id]}
          placeholder="New time block..."
          class="tk-new-entry-input"
          on:keydown={(e) => e.key === 'Enter' && handleAddEntry(tk.id)}
        />
        <button class="tk-add-entry-btn" on:click={() => handleAddEntry(tk.id)} title="Add entry"
