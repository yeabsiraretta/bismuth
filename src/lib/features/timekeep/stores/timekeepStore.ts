...
function loadSettings(): TimekeepSettings {
  try {
    ...
    return stored
      ? { ...DEFAULT_TIMEKEEP_SETTINGS, ...JSON.parse(stored) }
      : { ...DEFAULT_TIMEKEEP_SETTINGS };
  } catch {
    return { ...DEFAULT_TIMEKEEP_SETTINGS };
  }
}
      ...
      data: {
        entries: tk.data.entries.map((e) => {
          ...
          if (stopped.subEntries) {
            return {
              ...stopped,
              subEntries: stopped.subEntries.map((s) => (isRunning(s) ? stopEntry(s) : s)),
            };
          }
          return stopped;
        }),
