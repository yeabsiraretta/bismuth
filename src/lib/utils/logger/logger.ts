...
class Logger {
	...
	isConsoleEnabled(): boolean { return this.enableConsole; }
	...
	private consoleOutput(entry: LogEntry): void {
		...
		if (entry.error) {
			console.error(`%c  ${entry.error.name}: ${entry.error.message}`, 'color: #ef4444; font-weight: bold');
		}
	}
	...
	private buildErrorDetail(error: unknown): ErrorDetail | undefined {
		...
		if (error instanceof Error) {
			...
			return { name: error.name, message: error.message, stack: error.stack, cause: cause ? String(cause) : undefined };
		}
		if (typeof error === 'string') return { name: 'Error', message: error };
		if (typeof error === 'object') {
			const obj = error as Record<string, unknown>;
			return { name: String(obj['name'] || 'Error'), message: typeof obj['message'] === 'string' ? obj['message'] as string : JSON.stringify(error) };
		}
