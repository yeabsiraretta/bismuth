...
export function getWebVitals(): Record<string, number | null> {
  ...
  return {
    ...
    lcp: get('perf.lcp_ms'),
    cls: get('perf.cls'),
    ttfb: get('perf.ttfb_ms'),
    domInteractive: get('perf.dom_interactive_ms'),
    domComplete: get('perf.dom_complete_ms'),
    loadMs: get('perf.load_ms'),
    longTaskCount: get('perf.longtask.count'),
    longTaskTotalMs: get('perf.longtask.total_ms'),
    fetchCount: get('perf.fetch.count'),
  };
}
