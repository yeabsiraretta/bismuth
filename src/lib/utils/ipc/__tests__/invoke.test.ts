...
const mockLog = log as unknown as { info: ReturnType<typeof vi.fn>; error: ReturnType<typeof vi.fn>; interaction: ReturnType<typeof vi.fn> };
...
describe('ipcCall', () => {
  ...
  it('uses log.interaction on success', async () => {
    ...
    expect(mockLogInteraction).toHaveBeenCalledWith('ipc', 'do_thing', expect.objectContaining({ durationMs: expect.any(Number) }));
  });

  it('uses custom label in error message', async () => {
    mockInvoke.mockRejectedValue(new Error('err'));
    await expect(
      ipcCall('cmd', {}, { label: 'create widget' }),
