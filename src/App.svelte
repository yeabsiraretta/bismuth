...
<script lang="ts">
  ...
  let settingsInitialTab:
    'general' | 'editor' | 'appearance' | 'features' | 'vault' | 'hotkeys' | 'about' | undefined =
    undefined;
  ...
  $: {
    const all = [...visibleLeftTabs, ...visibleLeftLowerTabs];
    if (all.length > 0 && !all.find((t) => t.id === leftTab)) changeTab('left', all[0].id);
  }
  $: {
    const all = [...visibleRightTabs, ...visibleRightLowerTabs];
    if (all.length > 0 && !all.find((t) => t.id === rightTab)) changeTab('right', all[0].id);
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      /* running in browser dev — no Tauri */
    }
  });
  ...
  function handleLeftTabChange(tabId: string): void {
    ...
    if (tabId === 'open-tabs') {
      ...
      changeTab('left', 'open-tabs');
      return;
    }
    if ($viewportMode === 'canvas' || $viewportMode === 'graph') {
  ...
  function handleRightTabChange(tabId: string): void {
    ...
    if (tabId === 'calendar') {
      setViewportMode('calendar');
      return;
    }
    if ($viewportMode === 'calendar') {
      setViewportMode('note');
    }
  ...
  {:else}
    <div class="app-columns">
      <SidebarShell
        ...
        lowerTabs={visibleLeftLowerTabs}
        ...
        onSectionChange={(tabId: string, target: 'upper' | 'lower') =>
          moveTabToSection('left', tabId, target)}
      ...
      <main id="editor-main" class="editor-pane panel">
        {#if $viewportMode === 'rss'}
          <AsyncFeature
            featureId="rss"
            loader={() => import('@/features/rss').then((m) => ({ default: m.RssViewport }))}
          />
        {:else if $viewportMode === 'canvas'}
          <AsyncFeature
            featureId="canvas"
            loader={() => import('@/features/canvas/components/CanvasApp.svelte')}
          />
        {:else if $viewportMode === 'graph'}
          <AsyncFeature
            featureId="graph"
            loader={() => import('@/features/graph').then((m) => ({ default: m.GraphView }))}
          />
        {:else if $viewportMode === 'calendar'}
          <AsyncFeature
            featureId="calendar"
            loader={() => import('@/features/calendar').then((m) => ({ default: m.CalendarView }))}
          />
        {:else if $viewportMode === 'home'}
          <AsyncFeature
            featureId="hometab"
            loader={() => import('@/features/hometab').then((m) => ({ default: m.HomeTab }))}
          />
        {:else}
      ...
      <SidebarShell
        ...
        lowerTabs={visibleRightLowerTabs}
        ...
        onSectionChange={(tabId: string, target: 'upper' | 'lower') =>
          moveTabToSection('right', tabId, target)}
...
{#if $featureFlags['speed-reader']}
  <AsyncFeature
    ...
    loader={() => import('@/features/speedreader').then((m) => ({ default: m.SpeedReader }))}
  />
{/if}
