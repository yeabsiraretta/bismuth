...
pub fn git_status(state: &AppState) -> AppResult<GitStatus> {
...
pub fn git_stage_all(state: &AppState) -> AppResult<()> {
    let cwd = vault_path(state)?;
    run_git(&cwd, &["add", "-A"])?;
    Ok(())
}

pub fn git_commit(state: &AppState, message: &str) -> AppResult<()> {
    let cwd = vault_path(state)?;
    run_git(&cwd, &["commit", "-m", message])?;
    Ok(())
}

pub fn git_push(state: &AppState) -> AppResult<()> {
    let cwd = vault_path(state)?;
    run_git(&cwd, &["push"])?;
    Ok(())
}

pub fn git_pull(state: &AppState) -> AppResult<()> {
    let cwd = vault_path(state)?;
