...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct BackupListResult {
...
pub(crate) fn create_backup(state: &AppState) -> AppResult<BackupInfo> {
...
pub(crate) fn list_backups(state: &AppState) -> AppResult<BackupListResult> {
    let root = state.vault_root()?;
    let bak_dir = backup_dir(&root);
...
pub(crate) fn delete_backup(state: &AppState, backup_path: &str) -> AppResult<()> {
    let root = state.vault_root()?;
    let bak_dir = backup_dir(&root);
