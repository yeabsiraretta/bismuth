...
pub fn open_vault(state: &AppState, path: &str) -> AppResult<VaultResponse> {
    ...
    tracing::info!(vault_path = path, "Vault opened");
    Ok(VaultResponse {
        name,
...
pub fn create_vault(state: &AppState, path: &str, name: &str) -> AppResult<VaultResponse> {
    let root = Path::new(path);
    ...
    tracing::info!(vault_path = path, vault_name = name, "Vault created");
    Ok(VaultResponse {
        name: name.to_string(),
        root_path: path.to_string(),
    })
...
pub fn scan_vault(state: &AppState) -> AppResult<Vec<NoteMeta>> {
...
fn collect_notes(root: &Path, dir: &Path, notes: &mut Vec<NoteMeta>) -> AppResult<()> {
    ...
    for entry in entries {
        ...
        } else if matches!(path.extension().and_then(|e| e.to_str()), Some("md") | Some("canvas")) {
...
pub fn read_note(state: &AppState, rel_path: &str) -> AppResult<NoteResponse> {
    let root = state.vault_root()?;
...
pub fn write_note(state: &AppState, rel_path: &str, content: &str) -> AppResult<()> {
    let root = state.vault_root()?;
    let full = safe_join(&root, rel_path)?;
...
pub fn delete_note(state: &AppState, rel_path: &str) -> AppResult<()> {
    let root = state.vault_root()?;
    let full = safe_join(&root, rel_path)?;
    if !full.exists() {
...
pub fn rename_note(
    ...
    let new_filename = format!("{new_title}.md");
...
pub fn batch_read_notes(state: &AppState, paths: &[String]) -> AppResult<Vec<BatchNoteContent>> {
    let root = state.vault_root()?;
...
pub fn build_graph_data(state: &AppState) -> AppResult<GraphDataResult> {
...
pub fn extract_vault_tags(state: &AppState) -> AppResult<Vec<super::stats_service::TagCount>> {
    use std::collections::HashMap;
...
pub fn search_vault(state: &AppState, query: &str) -> AppResult<Vec<SearchResult>> {
    let root = state.vault_root()?;
    let mut notes = Vec::new();
    collect_notes(&root, &root, &mut notes)?;
...
mod tests {
    ...
    fn create_and_read_note() {
        let (_tmp, state) = setup();
        let created = create_note(&state, "Hello", None).unwrap();
    ...
    fn delete_note_removes_file() {
        let (_tmp, state) = setup();
        create_note(&state, "Removable", None).unwrap();
    ...
    fn rename_note_updates_path() {
        let (_tmp, state) = setup();
        create_note(&state, "OldName", None).unwrap();
    ...
    fn search_finds_matches() {
        let (_tmp, state) = setup();
        create_note(&state, "Searchable", None).unwrap();
    ...
    fn scan_finds_created_notes() {
        let (_tmp, state) = setup();
        create_note(&state, "Note1", None).unwrap();
        create_note(&state, "Note2", Some("subfolder")).unwrap();
        let notes = scan_vault(&state).unwrap();
    ...
    fn batch_read_returns_contents() {
        let (_tmp, state) = setup();
        create_note(&state, "Alpha", None).unwrap();
        write_note(&state, "Alpha.md", "alpha content").unwrap();
        create_note(&state, "Beta", None).unwrap();
    ...
    fn build_graph_extracts_links() {
        let (_tmp, state) = setup();
        create_note(&state, "PageA", None).unwrap();
        write_note(&state, "PageA.md", "# A\nSee [[PageB]] for details").unwrap();
        create_note(&state, "PageB", None).unwrap();
    ...
    fn scan_includes_bismuth_templates() {
        ...
        create_note(&state, "Regular", None).unwrap();
        let notes = scan_vault(&state).unwrap();
    ...
    fn extract_vault_tags_counts() {
        let (_tmp, state) = setup();
        create_note(&state, "Tagged", None).unwrap();
