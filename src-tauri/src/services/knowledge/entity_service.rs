...
impl PortentType {
    /// Parses a string into a `PortentType`, falling back to `Custom` for unknown values.
...
impl EntityService {
    ...
    pub fn load_custom_types(&mut self) -> Result<()> {
        ...
        let config_path = self.vault_root.join(VAULT_DIR_NAME).join("entity-types.json");
    ...
    pub fn get_entity_relationships(&self, note_path: &Path) -> Result<EntityRelationships> {
        super::entity_relationships::resolve_relationships(&self.vault_root, note_path)
    }
    ...
    pub fn detect_circular_relationships(&self, note_path: &Path) -> Result<Vec<Vec<String>>> {
        // Delegate to entity_relationships — return just circular_warnings
        let rels = super::entity_relationships::resolve_relationships(&self.vault_root, note_path)?;
        Ok(rels.circular_warnings)
    }
    ...
    fn derive_lifecycle(frontmatter: &std::collections::HashMap<String, serde_json::Value>) -> String {
        super::entity_relationships::derive_lifecycle(frontmatter)
    }
