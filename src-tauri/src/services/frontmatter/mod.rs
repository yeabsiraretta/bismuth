...
impl FrontmatterService {
    ...
    /// Gets a field from the `bismuth` namespace in frontmatter.
    /// Reads from `frontmatter["bismuth"][key]`.
    pub fn get_bismuth_field<'a>(frontmatter: &'a HashMap<String, Value>, key: &str) -> Option<&'a Value> {
        frontmatter
            .get("bismuth")
    ...
    /// Sets a field in the `bismuth` namespace.
    /// Creates the `bismuth` object if it doesn't exist.
    pub fn set_bismuth_field(
        frontmatter: &mut HashMap<String, Value>,
        key: &str,
        value: Value,
    ) {
        let bismuth = frontmatter
            .entry("bismuth".to_string())
            .or_insert_with(|| Value::Object(serde_json::Map::new()));
        if let Some(obj) = bismuth.as_object_mut() {
